...
|includeRelationships|IncludeRelationships.NONE|La réponse doit indiquer également les relations avec ce nœud. +
valeurs possibles:
...
|includeAcl|false|Si [green]*true*, la réponse doit indiquer également les `ACL` (**A**ccess **C**ontrol **L**ists) pour ce nœud
|===
