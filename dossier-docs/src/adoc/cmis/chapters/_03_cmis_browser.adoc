...
[source]
----
http(s)://<hostname>:<port>/<context>/cmis/browser/Dossier-5/root?objectId=ZmxvdzpDUg%3D%3D&cmisselector=children&succinct=true
----
...
Les opérations de crétion, de mise à jour ou de suppression sont effectuées via une commande *HTTP* de type *POST*.
...
[source]
----
http(s)://<hostname>:<port>/<context>/cmis/browser/Dossier-2?cmisselector=query&succinct=true&q=SELECT+cmis:objectId,dc:title+FROM+cmis:folder+WHERE+dc:title+=+'Workspaces'"
----
...
[source]
----
http(s)://<hostname>:<port>/<context>/cmis/browser/Dossier-5/root?objectId=YXR0YWNobWVudDpDUiNEZXJuaWVycyBjb21wdGUgcmVuZHVzIzQ2MzMjNDI1Mg%3D%3D&cmisselector=content
----
