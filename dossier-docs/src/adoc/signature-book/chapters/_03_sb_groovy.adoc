...
* *_scriptLogger*: le logger (Classe `org.slf4j.Logger`)
* *_userContext*: le user context (Classe `com.digitech.dossier.common.model.backend.UserCoreContext`)
* *_document*: le document Airs qui contient la pièce jointe envoyée vers le {signbook}. (Class `com.digitech.dossier.common.model.backend.airs.IDocument`)
* *_attachment*: la pièce jointe envoyée vers le {signbook}. (Class `com.digitech.dossier.common.model.backend.airs.IAttachment`)
* *_flowModel*: le code du flux auquel appartient le document (Class `java.lang.String`)

Dans ce script, deux _maps_ vont pouvoir être renseignées:

* *propertiesMap* : Map de propriétés qui vont permettre de modifier les valeurs par défaut de certaines informations utilisées pour l’envoi vers le {signbook}.
...
* *metadatasMap* : Map de *Metadatas*, ces informations sont transmises directement au {signbook}.
