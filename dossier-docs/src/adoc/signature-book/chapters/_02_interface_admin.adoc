...
[[sb_admin_rights]]
=== Droits complémentaires {arcadeged}
...
|===
|Droit|Description
|*DOSSIERS_SIGN_SEND*|{arcadeged}: Droit d'envoyer des documents vers le parapheur
|*DOSSIERS_SIGN_RETRIEVE*|{arcadeged}: Droit de relever manuellement les documents du parapheur
|*DOSSIERS_SIGN_CANCEL*|{arcadeged}: Droit d'annuler les documents du parapheur
|*DOSSIERS_SIGN_SHOW_HISTORY*|{arcadeged}: Droit de consulter l'historique du parapheur
|===


[[sb_admin_activation]]
=== Activation de la fonctionnalité {signbook} dans {arcadeged}
...
|*sign.airs.signature.enabled*|[green]#true#|Activation du module de signature OTP/{signbook}
...
[NOTE]
====
À partir de la version *7.2* de {arcadeged}, ces paramètres peuvent être directement définis via l'application {adminAIRS}
====