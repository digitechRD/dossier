...
Depuis l'application {ga}, veuillez créer un *type d’interface* {signbook}
...
Toujours depuis l'application {ga}, veuillez créer un connecteur
...
Depuis l'application {ga}, sélectionnez le menu *_Interfaces_*

.{ga}, _Menu Interfaces_
...
[NOTE]
====
A partir de la verison *7.2* de {dossier}, ces paramètres peuvent être directement définis via l'application {ga}
====