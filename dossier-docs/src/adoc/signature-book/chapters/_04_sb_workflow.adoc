...
[source,java]
----
resultValue.getPropertiesMap().put(EnumDepth.DEPTH_0.name(), CIRCUIT_NAME) // here set the workflow
----
...
En plus du droit *DOSSIERS_SIGN_SEND* que doit posséder l’utilisateur, l’envoi ne pourra être effectué que si le document est en mode *Edition*
...
commentaires qui ont pu y être ajoutés. Ce bouton n’est actif que si l’utilisateur a le droit *DOSSIERS_SIGN_RETRIEVE* et si le document est en consultation.
...
*DOSSIERS_SIGN_CANCEL* et si le document est en consultation.

Si l’utilisateur a le droit *DOSSIERS_SIGN_SHOW_HISTORY*, il aura également accès à l’historique des actions liées au parapheur.
