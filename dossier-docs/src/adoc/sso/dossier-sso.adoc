...
* *keyTabLocation*: nom du `keytab` (localisé dans le répertoire *xml* de {dossier})
* *servicePrincipal*: service principal utilisé lors de la génération du `keytab`
* *debug*: paramètre optionnel pour augmenter les traces  (*true* ou *false*).
* *allowLogin*: indique si la mire de login classique (user/password) reste disponible.

.exemple
[source,xml]
----
 <spnegoConfig servicePrincipal="HTTP/SRV-APPDIGI-01@DIGITECH.LAN" keytabLocation="kerb5_dossier.ktab" debugMode="true" allowLogin="true"/>
----
