...
[[_99_appendixes_variable_available]]
==== Tester si une variable est disponible/présente

Pour quelques raisons que ce soient, vous pouvez être amené à vouloir tester si une variable est présente.

Pour cela, le contexte *binding* du script {groovy} peut être testé comme il suit

[source, groovy]
--
// test si la variable *_userContext* est disponible
  if(binding.variables.containsKey("_userContext")) {
    _scriptLogger("La variable \"_userContext\" est disponible (user name $_userContext.userFullName")
  }
  else {
...
pass:[<a href="https://docs.groovy-lang.org/latest/html/documentation/index.html#_string_interpolation" target="_blank">documentation</a>],
