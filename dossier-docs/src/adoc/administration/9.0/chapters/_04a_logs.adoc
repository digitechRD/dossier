...
==== Modification via les apis {rest} d'exploitation

Depuis la version *9.0* d'{arcadeged}, il est également possible de modifier _temporairement_ (c'est-à-dire jusqu'au prochain *[underline]#redémarrage#* de
l'application) le niveau de logs de l'application

image::./04_logs/ws_log_setup.png[]

[TIP]
====
Veuillez vous référer au manuel décrivant les APIs {rest} pour obtenir toutes les informations sur cette fonctionnalité.
====
