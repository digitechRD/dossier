...
(((Wolpertinger))) (((Ravenous Beast,Wolpertinger))) You may not be familiar with these {uri-wolpertinger}[ravenous beasts].
...
"`An odoriferous one or a mineral one?`" Kizmet shrugged.
...
How about some source code that styles code?
So meta!
...
[verse]
First, we need to waste several lines using a verse to push the next block to its breaking point.
