...
log.debug("In facture_initPanelChangerCDGActionUnitView.groovy v1.0 (id [" + theDocument.getAirsRefId() + "])");

// Get the user list
List<User> users = new ArrayList<User>(); 
log.debug("Script triggered on init custom Modal Panel : récupération de la liste des utilisateurs traitants");
users = ScriptUtils.getUsersWithProfil("PF_CDG") ;
...
customActionModel.setModalPanelTitle("Sélection d'un nouveau controleur de gestion pour remplacer :"+UserName);
...
log.debug("Out facture_initPanelChangerCDGActionUnitView.groovy");
