...
int oldUserId =  FieldUtils.getValue(theDocument, "FACT_USR_ACHAT");
...
if(!comment.equals("")) {
	...
	log.debug("Script : facture_ChangerAchatActionUnitView.groovy : Enregistrement du commentaire : "+comment);
	ScriptUtils.getDocumentMgr().updateDocumentComments(usrContext, theDocument);

  //Changement du user Achat
  FieldUtils.setValue(theDocument, "FACT_USR_ACHAT", newUserId);
  ...
  //ScriptUtils.addHistoForField(theDocument, usrContext, "Changement gestionaire Achat ", "",  oldValue, newValue);
  ScriptUtils.addHistoWrkNewUser(theDocument, usrContext, "Changement gest achat",  oldValue, newValue);
  

  result.setMessageSeverity(  com.digitech.dossier.script.model.IScriptResultValueModel.Severity.INFO );
  result.setMessageSummary("Mise à jour du gestionaire Achat");
  ...
  log.debug("Out1 facture_ChangerAchatActionUnitView.groovy ");

	
}
else {
	log.debug("Script : facture_ChangerAchatActionUnitView.groovy : Aucun commentaire");
  ...
  log.debug("Out2 facture_ChangerAchatActionUnitView.groovy ");
