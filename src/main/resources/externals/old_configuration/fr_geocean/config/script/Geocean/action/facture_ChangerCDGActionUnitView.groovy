...
int oldUser1Id =  FieldUtils.getValue(theDocument, "FACT_USR_CTRLGEST1");
int oldUser2Id =  FieldUtils.getValue(theDocument, "FACT_USR_CTRLGEST2");
int oldUser3Id =  FieldUtils.getValue(theDocument, "FACT_USR_CTRLGEST3");
String FieldName;
String Name;
int nNrGest=0;
...
if(CurentUserId == oldUser1Id)   {
  nNrGest=1;
  FieldName = "FACT_USR_CTRLGEST1";
  Name = "Controleur de gestion 1";
}
else if(CurentUserId == oldUser2Id)   { 
  nNrGest=2;
  FieldName = "FACT_USR_CTRLGEST2";
  Name = "Controleur de gestion 2";
}
else if(CurentUserId == oldUser3Id)   { 
  nNrGest=3;
  FieldName = "FACT_USR_CTRLGEST3";
  Name = "Controleur de gestion 3";
}
...
//  ScriptUtils.addHistoForField(theDocument, usrContext, "Changement "+Name, "",  oldValue, newValue);
  ...
  if(!comment.equals("")) {
  	...
  	log.debug("Script : facture_ChangerCdgActionUnitView.groovy : Enregistrement du commentaire : "+comment);
  	ScriptUtils.getDocumentMgr().updateDocumentComments(usrContext, theDocument);
  }
  else {
  	log.debug("Script : facture_ChangerCdgActionUnitView.groovy : Aucun commentaire");
  }
  ...
  log.debug("Out facture_ChangerCdgActionUnitView.groovy ");
...
// pas de user gestioneaire correspondant à l'utilisateur courrant !!!! 
  ...
  result.setMessageSummary("Impossible de mettre à jour le Controleur de gestion");
  result.setMessageDetail("L'utilisateur courant n'existe pas dans la liste des contrôleurs de gestion");
...
// pas de user gestioneaire correspondant à l'utilisateur courrant !!!! 
  ...
  log.debug("Out2 facture_ChangerCdgActionUnitView.groovy ");
