...
//         directeur" sur un type de document Facture
// Descritpions :
//  - 
// ****************************************************************************
...
log.debug("In facture_ChangerDIRVisibilityUnitView.groovy v1.0 (id [" + theDocument.getAirsRefId() + "])");
...
{
	if (afficheLesLog)
		log.debug("facture_ChangerDIRVisibilityUnitView.groovy : fieldState [" + fieldState + "]");

	if (fieldState.equals(ScriptUtils.getConstant("CODE_VALID_DIR")))
	{
		if (afficheLesLog)
			log.debug("facture_ChangerDIRVisibilityUnitView.groovy : cas validation DIR");

		if (UserUtils.hasProfile(usrContext, ScriptUtils.getConstant("PROFIL_DIR")))
		{
			Integer valideurId;

			valideurId = FieldUtils.getValue(theDocument, ScriptUtils.getConstant("FIELD_FACT_USR_DIR"));

			if (afficheLesLog)
				log.debug("facture_ChangerDIRVisibilityUnitView.groovy : valideurId [" + valideurId + "]");

			if (usrContext.getUser().getId() == valideurId)
			{
				if (afficheLesLog)
					log.debug("facture_ChangerDIRVisibilityUnitView.groovy : utilisateur selectionne dans le champ");
	...
	{
		log.info("facture_ChangerDIRVisibilityUnitView.groovy : cas inconnu ou non gere par le script");
	}
}
else
{
	log.debug("facture_ChangerDIRVisibilityUnitView.groovy : fieldState empty !");
}
...
if (boutonVisible){
	...
	log.debug("Out facture_ChangerDIRVisibilityUnitView.groovy : Visible");
} else {
	result.setValid(false);
	log.debug("Out facture_ChangerDIRVisibilityUnitView.groovy : Invisible");
}
