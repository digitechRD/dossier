...
log.debug("In facture_initPanelChangerDirActionUnitView.groovy v1.0 (id [" + theDocument.getAirsRefId() + "])");

// Get the user list
List<User> users = new ArrayList<User>(); 
log.debug("Script triggered on init custom Modal Panel : récupération de la liste des utilisateurs traitants");
users = ScriptUtils.getUsersWithProfil("PF_DIR") ;
...
customActionModel.setModalPanelTitle("Sélection d'un nouveau valideur de la Direction pour cette facture");
...
log.debug("Out facture_initPanelChangerDirActionUnitView.groovy");
