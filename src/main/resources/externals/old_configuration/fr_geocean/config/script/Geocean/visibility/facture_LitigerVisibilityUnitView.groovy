...
// Objet : Script qui permet d'afficher ou non le bouton d'action "Litige" 
...
log.debug("In facture_LitigerVisibilityUnitView.groovy v1.0 (id [" + theDocument.getAirsRefId() + "])");
...
{
	if (afficheLesLog)
		log.debug("facture_LitigerVisibilityUnitView.groovy : fieldState [" + fieldState + "]");

	if (fieldState.equals(ScriptUtils.getConstant("CODE_CMPT_PREPA_PAIE")))
	{
		if (afficheLesLog)
			log.debug("facture_LitigerVisibilityUnitView.groovy : cas Compta Paie");

		if (UserUtils.hasProfile(usrContext, ScriptUtils.getConstant("PROFIL_CMPT")))
		{
			if (afficheLesLog)
				log.debug("facture_LitigerVisibilityUnitView.groovy : utilisateur avec le profil Compta");
	...
	{
		log.info("facture_LitigerVisibilityUnitView.groovy : cas inconnu ou non gere par le script");
	}
}
else
{
	log.debug("facture_LitigerVisibilityUnitView.groovy : fieldState empty !");
}
...
if (boutonVisible){
	...
	log.debug("Out facture_LitigerVisibilityUnitView.groovy : Visible");
} else {
	result.setValid(false);
	log.debug("Out facture_LitigerVisibilityUnitView.groovy : Invisible");
}
