...
// Objet : Script qui permet d'afficher ou non le bouoton d'action "Valider" 
...
log.debug("In facture_ValiderVisibilityUnitView.groovy v1.0 (id [" + theDocument.getAirsRefId() + "])");
...
{
	if (afficheLesLog)
		log.debug("facture_ValiderVisibilityUnitView.groovy : fieldState [" + fieldState + "]");
	...
	{
		if (afficheLesLog)
			log.debug("facture_ValiderVisibilityUnitView.groovy : cas Compta [" + fieldState + "]");

		if (UserUtils.hasProfile(usrContext, ScriptUtils.getConstant("PROFIL_CMPT")))
		{
			if (afficheLesLog)
				log.debug("facture_ValiderVisibilityUnitView.groovy : utilisateur avec le profil Compta");
	...
	{
		if (afficheLesLog)
			log.debug("facture_ValiderVisibilityUnitView.groovy : cas validation achat");
		...
		if (UserUtils.hasProfile(usrContext, ScriptUtils.getConstant("PROFIL_ACHAT")))
		{
			...
			if (afficheLesLog)
				log.debug("facture_ValiderVisibilityUnitView.groovy : valideurAchatId [" + valideurAchatId + "]");

			if (usrContext.getUser().getId() == valideurAchatId)
			{
				if (afficheLesLog)
					log.debug("facture_ValiderVisibilityUnitView.groovy : utilisateur selectionne dans le champ");
	...
	{
		if (afficheLesLog)
			log.debug("facture_ValiderVisibilityUnitView.groovy : cas validation ctrl gestion");
		...
		if (UserUtils.hasProfile(usrContext, ScriptUtils.getConstant("PROFIL_CTRLGEST")))
		{
			...
			if (afficheLesLog)
				log.debug("facture_ValiderVisibilityUnitView.groovy : valideurCdgId(s) [" + valideurCdg1Id + "/" + valideurCdg2Id + "/" + valideurCdg3Id + "]");
	...
	{
		if (afficheLesLog)
			log.debug("facture_ValiderVisibilityUnitView.groovy : cas validation cdp");
		...
		if (UserUtils.hasProfile(usrContext, ScriptUtils.getConstant("PROFIL_CDP")))
		{
			...
			if (afficheLesLog)
				log.debug("facture_ValiderVisibilityUnitView.groovy : valideurCdpId(s) [" + valideurCdp1Id + "/" + valideurCdp2Id + "/" + valideurCdp3Id + "]");
	...
	{
		if (afficheLesLog)
			log.debug("facture_ValiderVisibilityUnitView.groovy : cas validation daf");

		if (UserUtils.hasProfile(usrContext, ScriptUtils.getConstant("PROFIL_DAF")))
		{
			if (afficheLesLog)
				log.debug("facture_ValiderVisibilityUnitView.groovy : utilisateur avec le profil Daf");
	...
	{
		if (afficheLesLog)
			log.debug("facture_ValiderVisibilityUnitView.groovy : cas validation dir");
		...
		if (UserUtils.hasProfile(usrContext, ScriptUtils.getConstant("PROFIL_DIR")))
		{
			...
			if (afficheLesLog)
				log.debug("facture_ValiderVisibilityUnitView.groovy : valideurDirecteurId [" + valideurDirecteurId + "]");
	...
	{
		log.info("facture_ValiderVisibilityUnitView.groovy : cas inconnu ou non gere par le script");
	}
}
else
{
	log.debug("facture_ValiderVisibilityUnitView.groovy : fieldState empty !");
}
...
if (boutonVisible){
	...
	log.debug("Out facture_ValiderVisibilityUnitView.groovy : Visible");
} else {
	result.setValid(false);
	log.debug("Out facture_ValiderVisibilityUnitView.groovy : Invisible");
}
