...
log.debug("In facture_initPanelChangerAchatActionUnitView.groovy v1.0 (id [" + theDocument.getAirsRefId() + "])");

// Get the user list
List<User> users = new ArrayList<User>(); 
log.debug("Script triggered on init custom Modal Panel : récupération de la liste des utilisateurs traitants");
users = ScriptUtils.getUsersWithProfil("PF_ACHAT") ;
...
customActionModel.setModalPanelTitle("Sélection d'un nouveau gestionnaire achat pour cette facture");
...
log.debug("Out facture_initPanelChangerAchatActionUnitView.groovy");
