...
//         contrôleur de gestion" sur un type de document Facture
// Descritpions :
//  - 
// ****************************************************************************
...
log.debug("In facture_ChangerCDGVisibilityUnitView.groovy v1.0 (id [" + theDocument.getAirsRefId() + "])");
...
{
	if (afficheLesLog)
		log.debug("facture_ChangerCDGVisibilityUnitView.groovy : fieldState [" + fieldState + "]");

	if (fieldState.equals(ScriptUtils.getConstant("CODE_VALID_CTRLGEST")))
	{
		if (afficheLesLog)
			log.debug("facture_ChangerCDGVisibilityUnitView.groovy : cas validation CDG");
		...
		if (UserUtils.hasProfile(usrContext, ScriptUtils.getConstant("PROFIL_CTRLGEST")))
		{
			for (int i=1;i<4;i++)
			{
				...
				valideurId = FieldUtils.getValue(theDocument, ScriptUtils.getConstant("FIELD_FACT_USR_CTRLGEST" + i));

				if (afficheLesLog)
					log.debug("facture_ChangerCDGVisibilityUnitView.groovy : valideurId [" + valideurId + "/" + i + "]");

				if (usrContext.getUser().getId() == valideurId)
				{
					if (afficheLesLog)
						log.debug("facture_ChangerCDGVisibilityUnitView.groovy : utilisateur selectionne dans le champ [" + i + "]");
	...
	{
		log.info("facture_ChangerCDGVisibilityUnitView.groovy : cas inconnu ou non gere par le script");
	}
}
else
{
	log.debug("facture_ChangerCDGVisibilityUnitView.groovy : fieldState empty !");
}
...
if (boutonVisible){
	...
	log.debug("Out facture_ChangerCDGVisibilityUnitView.groovy : Visible");
} else {
	result.setValid(false);
	log.debug("Out facture_ChangerCDGVisibilityUnitView.groovy : Invisible");
}
