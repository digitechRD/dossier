...
logger.debug("Script field initialization: facture_init_readOnlyIfNo_AdminComptaDAF.groovy --- Start");
...
if(!UserUtils.hasProfile(usrContext, PROFIL_ADMIN_CODE)) {
  ...
  boolean IsUserHaveProfilDaf = UserUtils.hasProfile(usrContext, PROFIL_DAF_CODE);
 
  if(!IsUserHaveProfilCompta && !IsUserHaveProfilDaf ) {
    theOutput.getValue().getProperties().put(FieldProperty.READ_ONLY, "true");
  }

}
logger.debug("Script field initialization: facture_init_readOnlyIfNo_AdminComptaDAF.groovy --- End");
