...
logger.debug("Script field initialization: facture_init_readOnlyIfNo_AdminCompta.groovy --- Start");
...
if(!UserUtils.hasProfile(usrContext, PROFIL_ADMIN_CODE)) {
  ...
  if(!IsUserHaveProfilCompta) {
    theOutput.getValue().getProperties().put(FieldProperty.READ_ONLY, "true");
  }

}
logger.debug("Script field initialization: facture_init_readOnlyIfNo_AdminCompta.groovy --- End");
