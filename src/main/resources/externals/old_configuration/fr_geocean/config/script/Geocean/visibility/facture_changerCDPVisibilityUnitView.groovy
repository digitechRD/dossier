...
//         Chef De Projet" sur un type de document Facture
// Descritpions :
//  - 
// ****************************************************************************
...
log.debug("In facture_ChangerCDPVisibilityUnitView.groovy v1.0 (id [" + theDocument.getAirsRefId() + "])");
...
{
	if (afficheLesLog)
		log.debug("facture_ChangerCDPVisibilityUnitView.groovy : fieldState [" + fieldState + "]");

	if (fieldState.equals(ScriptUtils.getConstant("CODE_VALID_CDP")))
	{
		if (afficheLesLog)
			log.debug("facture_ChangerCDPVisibilityUnitView.groovy : cas validation CDP");
		...
		if (UserUtils.hasProfile(usrContext, ScriptUtils.getConstant("PROFIL_CDP")))
		{
			for (int i=1;i<4;i++)
			{
				...
				valideurId = FieldUtils.getValue(theDocument, ScriptUtils.getConstant("FIELD_FACT_USR_CDP" + i));

				if (afficheLesLog)
					log.debug("facture_ChangerCDPVisibilityUnitView.groovy : valideurId [" + valideurId + "/" + i + "]");

				if (usrContext.getUser().getId() == valideurId)
				{
					if (afficheLesLog)
						log.debug("facture_ChangerCDPVisibilityUnitView.groovy : utilisateur selectionne dans le champ [" + i + "]");
	...
	{
		log.info("facture_ChangerCDPVisibilityUnitView.groovy : cas inconnu ou non gere par le script");
	}
}
else
{
	log.debug("facture_ChangerCDPVisibilityUnitView.groovy : fieldState empty !");
}
...
if (boutonVisible){
	...
	log.debug("Out facture_ChangerCDPVisibilityUnitView.groovy : Visible");
} else {
	result.setValid(false);
	log.debug("Out facture_ChangerCDPVisibilityUnitView.groovy : Invisible");
}
