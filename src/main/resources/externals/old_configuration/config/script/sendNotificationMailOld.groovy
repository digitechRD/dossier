...
log.error("sendNotificationMail : IN");
...
log.error("Lecture du parametre d'entree fac_etat="+strEtatFac);



String MAIL_TEMPLATE_FOLDER = "config/templates/mail_template";
log.error("MAIL_TEMPLATE_FOLDER : "+MAIL_TEMPLATE_FOLDER);
...
log.error("Valeur de l'etat : "+strEtatFacForAIRSRequest);
log.error("Id de l'etat recherché : "+nTermIDForEtat);
log.error("Id de l'etat recherché : "+nTermIDForEtat);
...
log.error("on effectue la recherche AIRS");
...
if(documentList!=null) {
	log.error(documentList.size()+" facture(s) sont dans l'etat : "+strEtatFac);
  ...
  for(IDocument facture : documentList) {
    try {
		...
		log.error("DocID="+String.valueOf(docId));
  ...
  //Envoi du mail
	try {
		...
		log.error("Récupération du template mail");
		...
		if (!UserMailList.isEmpty()) {
			...
			for (Iterator<Integer> id = UserMailList.keySet().iterator() ; id.hasNext() ;){
				...
				log.error("Envoi d'un mail à : "+key);
			  ...
			  for(Integer idDeleg : listDeleg) { 
			    log.error("J'envoit aussi un mail à : "+idDeleg); 
  ...
  log.error("Aucune facture n'est dans l'état "+strEtatFac);
}

log.error("sendNotificationMail : OUT");
...
List<Integer> getDelegTemporaire(Integer idUser)
{
  ...
  Logger log = LoggerFactory.getLogger(this.getClass());
  log.error("Récupération des id des délégués de : "+idUser);
  try {
     for(IDelegate delegate : getUserMgr().getDelegates(UserUtils.getAdminUserContext().getJeton(), idUser)) {
	  ...
	  log.error("Date début délégation : "+startDate);
	  log.error("Date fin délégation : "+endDate);
      boolean bDelegationActif = false;
      if ( startDate != null && endDate != null )
      {       
         Date dateDuJour=new Date(); 
		 log.error("Date du jour : "+dateDuJour);
         if ( dateDuJour.before(endDate) && dateDuJour.after(startDate)) 
		 {
            bDelegationActif = true;
			log.error("OK, c'est un délégué temporaire : "+user.getId());
		 }
      }
      
      if(user != null && bDelegationActif) {
			    log.error("J'ajoute aussi : "+user.getId()); 
...
log.error("userId in sendMailToUser="+userId);
  ...
  for (Iterator<List> liste = listeFacture.values().iterator() ; liste.hasNext() ;){
   ...
   for(IDocument facture : liste.next()) {
      //Récupération du Doc_Id
		  Integer docId =  facture.getAirsRefId();
		  log.error("DocIDInMail="+String.valueOf(docId));
      theDocumentUserList.add(facture);
    }
  }
  log.error("ApplicationUtils.sendMail : BEFORE");
  ApplicationUtils.sendMail(
    ...
    null);    
  log.error("ApplicationUtils.sendMail : AFTER");
}
