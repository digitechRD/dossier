 ...
 * Author : JMU
 * Date : 22/06/12
 * Description : Ce script permet de mettre � jour l'environnement CIPRES pour les factures � payer. Lanc� par le taskManager.
 * IN : scriptLogger/
 *
 */
...
//r�cup�ration de toutes les factures dans l'�tat Bon A payer
...
if(documentList != null) {
  ...
  for(IDocument facture : documentList) {
    ...
    if(sCodeSoc == null || sNumFac == null || sMatricule == null || sDateFacture == null) {
      //erreur, les informations de la facture sont erronn�es
      log.error("Les donn�es de la facture sont erron�es. Code Soci�t� : " + sCodeSoc + " Num Facture : " + sNumFac + " Matricule : " + sMatricule + "sDateFacture :" + sDateFacture);
      ...
      if(bUpdateCIPRES == true) {
        //on passe la facture � l'�tat R�gl�e dans AIRS DOSSIER
        log.info("MAJCIPRES : la mise � jour CIPRES s'est correctement effectu�e, la facture passe dans l'�tat Attente Reglement dans AIRS DOSSIER");
        ...
        //probl�me lors de la mise � jour CIPRES
        log.info("MAJCIPRES : Probl�me lors de la mise � jour de CIPRES : le syst�me n'est pas coh�rent. La facture reste dans l'�tat BonAPayer dans AIRS DOSSIER");
...
//r�cup�ration de toutes les factures dans l'�tat Attente Regl
...
if(documentList2 != null) {
  ...
  for(IDocument facture : documentList2) {
    ...
    if(sCodeSoc == null || sNumFac == null || sMatricule == null || sDateFacture == null) {
      //erreur, les informations de la facture sont erronn�es
      log.error("Les donn�es de la facture sont erron�es. Code Soci�t� : " + sCodeSoc + " Num Facture : " + sNumFac + " sMatricule : " + sMatricule + "sDateFacture :" + sDateFacture);
      addcomment(facture, "V�rification facture lettree : Num�ro de facture ou Code societe ou Matricule ou Date Facture non correct", userContext, documentMgr2, log);
    }
    else {
      boolean bLettree = estLettree(sCodeSoc, sNumFac, sMatricule, sDateFacture, log);
      if(bLettree == true) {
        log.info("MAJCIPRES : La facture a �t� pay�e, on la passe dans l'�tat r�gl�e");
...
public boolean estLettree(String sCodeSoc, String sNumFact, String sMatricule, String sDateFacture, Logger log) {
  ...
  if(StringUtils.isNotBlank(sCodeSoc) && StringUtils.isNotBlank(sNumFact) && StringUtils.isNotBlank(sMatricule) && StringUtils.isNotBlank(sDateFacture)) {
    ...
    // On v�rifie que le pool dde connexion n'existe pas d�ja
    ...
    try {
      ...
      if(rsSet != null) {
        while(rsSet.next()) {
          ...
          bFind = true;
          log.info("Script MAJCIPRES : facture trouv�e, lettr�e! : " + sMovInvoice);
        }
        rsSet.close();
      }
      dbStatement.close();
  ...
  if(StringUtils.isNotBlank(sCodeSoc) && StringUtils.isNotBlank(sNumFact) && StringUtils.isNotBlank(sMatricule) && StringUtils.isNotBlank(sDateFacture)) {
    ...
    try {
      ...
      if(rsSet != null) {
        while(rsSet.next()) {
          nNb = rsSet.getInt("NB");
          log.info("Script MAJCIPRES : Nombre de facture impact�es select 1 dans PIECES : " + nNb);
        }
        rsSet.close();
      }
      ...
      if(nNb == 1) {
        ...
        if(rsSet != null) {
          while(rsSet.next()) {
            nNb = rsSet.getInt("NB");
            log.info("Script MAJCIPRES : Nombre de facture impact�es select 2 dans MOV : " + nNb);
          }
          rsSet.close();
        }
        ...
        if(nNb >= 1) {
          ...
          Integer nNbUpdate = 1;
          log.info("Script MAJCIPRES : Nombre de Update request1 (recette toujours � 1): " + nNbUpdate);
          if(nNbUpdate == 1) {
            //MODIFICATION POUR L'ENVIRONNEMENT DE RECETTE
            //nNbUpdate = dbStatement.executeUpdate(request2);
            nNbUpdate = 1;
            log.info("Script MAJCIPRES : Nombre de Update request2 (recette toujours � 1) : " + nNbUpdate);
            if(nNbUpdate >= 1) {
              log.info("Script MAJCIPRES : TOUT EST OK, la mise � jour CIPRES est effective");
              bUpdateOK = true;
              //dbConnection.commit();
            }
          }
          ...
          log.info("Script MAJCIPRES : Nombre de facture impact�es select 2 incorrect : " + nNb);
        }
      }
      else {
        log.info("Script MAJCIPRES : Nombre de facture impact�es select 1 incorrect : " + nNb);
      }
    }
    catch(DigiInternalException e) {
      // TODO Auto-generated catch block
...
void addcomment(com.digitech.dossier.common.model.backend.airs.impl.Document document, String commentToAdd, UserCoreContext userContext, com.digitech.dossier.common.service.IDocument documentMgr, org.slf4j.impl.Log4jLoggerAdapter log) {
  /*IComment commentModel = new Comment();
  commentModel.setComment(commentToAdd);
  try {
	//on supprime les commentaires du m�me type si il y en a d�j�
