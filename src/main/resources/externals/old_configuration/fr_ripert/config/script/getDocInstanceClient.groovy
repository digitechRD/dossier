		...
		try {
			...
			if(childIds != null) {
				for(Integer childId : childIds) 
                {
                    ...
                    IField fieldEtatInstance_Doc = docToAdd.getField(EN_INSTANCE_DOC_FIELD_CODE);  
	               
                    String fieldEtatInstancevalue = fieldEtatInstance_Doc.getValue( );
	                if( fieldEtatInstancevalue!=null && fieldEtatInstancevalue.compareToIgnoreCase(EN_INSTANCE_DOC_OUI) == 0 )
	                {
