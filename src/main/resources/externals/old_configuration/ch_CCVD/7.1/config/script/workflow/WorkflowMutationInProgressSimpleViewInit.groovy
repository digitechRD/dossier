...
/*************************************************************************************************
 * 							    Validation étape workflow Mutation en progession - INIT
 **************************************************************************************************
 Date : 12.11.2014
 Auteur : MTO

 Description : Permet la validation du workflow en vue simple de la tâche Mutation en progession
 **************************************************************************************************/

_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW MUTATION IN PROGRESS SIMPLE VIEW INIT - START")
...
// Variable locale
String TASKS_VALIDATION_NAME = "TraiterCTE,MutationCTE,ArchiveCTE"
...
try {
  ...
  _scriptLogger.error("[CUSTOM ACTION] - WorkflowMutationInProgressSimpleView - ERREUR : ", e)
  return
}
    ...
    if(Constants.DOC_LOCKED_BY_OTHER == currentDoc.getLockType()) states.put(currentDoc.getAirsRefId(), "Document vérouillé")
      ...
      for(IWFTaskModel iwfTaskModel : wfTasks) {
        ...
        if(TASKS_VALIDATION_NAME.contains(iwfTaskModel.getName())) {
          ...
          for(WFActor actor : iwfTaskModel.getActors()) {
            ...
            if(actor.getId().startsWith("airs") && tasks.get(currentDoc.getAirsRefId()) == null) {
              ...
              if(actor.getId().substring(5).equalsIgnoreCase(userContext.getUser().getId().toString())) {
                ...
                outputs.put(currentDoc.getAirsRefId(), Constants.AKAZI_OUTPUT_MUTATION_IN_PROGRESS)
                states.put(currentDoc.getAirsRefId(), "Le document peut être mis en mutation en cours")
                break
              }
              else {
          ...
          states.put(currentDoc.getAirsRefId(), "Le document ne peut pas être mis en mutation en cours")
        }
      }
    }
  }
...
_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW MUTATION IN PROGRESS SIMPLE VIEW INIT - END")