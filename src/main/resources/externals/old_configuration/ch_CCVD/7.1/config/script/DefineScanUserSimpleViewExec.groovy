...
_scriptLogger.debug("[CUSTOM ACTION] - DEFINE SCANNER USER SIMPLE VIEW EXEC - START")
...
try {
  ...
  _scriptLogger.error("[CUSTOM ACTION] - DefineScannerUserSimpleViewExec - ERREUR : ", e.localizedMessage)
  return
}
...
try {
  if(data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
    _scriptLogger.debug("[CUSTOM ACTION] - DEFINE SCANNER USER SIMPLE VIEW EXEC - END")
    return
  }
  ...
  _scriptLogger.error("[CUSTOM ACTION] - DefineScannerUserSimpleViewExec - ERREUR : ", e)
  result.setMessageSeverity(IScriptResultValueModel.Severity.ERROR)
