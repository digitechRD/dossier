...
/*************************************************************************************************
 * 							    Validation étape workflow Créé NSS CICA - INIT
 **************************************************************************************************
 Date : 12.11.2014
 Auteur : MTO

 Description : Permet la validation du workflow en vue simple de la tâche Créé NSS CICA
 **************************************************************************************************/

_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW CREATE NSS CICA SIMPLE VIEW INIT - START")
...
try {
  ...
  _scriptLogger.error("[CUSTOM ACTION] - WorkflowCreateNSSCICASimpleViewInit - ERREUR : ", e)
  return
}
...
try {
    ...
    if(Constants.DOC_LOCKED_BY_OTHER == currentDoc.getLockType()) states.put(currentDoc.getAirsRefId(), "Document vérouillé")
      ...
      for(IWFTaskModel iwfTaskModel : wfTasks) {
        ...
        if(TASKS_VALIDATION_NAME.contains(iwfTaskModel.getName())) {
          ...
          for(WFActor actor : iwfTaskModel.getActors()) {
            ...
            if(actor.getId().startsWith("airs") && tasks.get(currentDoc.getAirsRefId()) == null) {
              ...
              if(actor.getId().substring(5).equalsIgnoreCase(userContext.getUser().getId().toString())) {
                ...
                states.put(currentDoc.getAirsRefId(), "La réponse de la centrale peut être validée")
                break
              }
              else {
          ...
          states.put(currentDoc.getAirsRefId(), "La réponse de la centrale ne peut pas être validée")
        }
      }
    }
  }
  ...
  data.put("GROOVYNAME", "WorkflowCreateNSSCICASimpleViewInit")
...
_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW CREATE NSS CICA SIMPLE VIEW INIT - END")