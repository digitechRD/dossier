...
/*************************************************************************************************
 * 							    Validation étape workflow Adresse créée - INIT
 **************************************************************************************************
 Date : 12.11.2014
 Auteur : MTO

 Description : Permet la validation du workflow en vue simple de la tâche Adressé créée
 **************************************************************************************************/

_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW ADRESS CREATED SIMPLE VIEW INIT - START")
...
// Variable locale
String TASKS_VALIDATION_NAME = "AdresseACreer"

try {
  data = Utils.getCustomActionController().getModel().getModalPanelModel()
  ...
  _scriptLogger.error("[CUSTOM ACTION] - WorkflowAdressCreatedSimpleView - ERREUR : ", e)
  return
}
    ...
    if(Constants.DOC_LOCKED_BY_OTHER == currentDoc.getLockType()) {
      ...
      for(IWFTaskModel iwfTaskModel : wfTasks) {
        ...
        if(TASKS_VALIDATION_NAME.contains(iwfTaskModel.getName())) {
          ...
          for(WFActor actor : iwfTaskModel.getActors()) {
            ...
            if(actor.getId().startsWith("airs") && tasks.get(currentDoc.getAirsRefId()) == null) {
              ...
              if(actor.getId().substring(5).equalsIgnoreCase(userContext.getUser().getId().toString())) {
                ...
                outputs.put(currentDoc.getAirsRefId(), Constants.AKAZI_OUTPUT_ADRESS_CREATED)
                states.put(currentDoc.getAirsRefId(), "La création de l'adresse peut être validée")
                break
              }
              else {
              ...
              tasks.put(currentDoc.getAirsRefId(), iwfTaskModel)
              outputs.put(currentDoc.getAirsRefId(), Constants.AKAZI_OUTPUT_ADRESS_CREATED)
              states.put(currentDoc.getAirsRefId(), "La creation de l'adresse peut etre validée")
              break
            }
            else {
              states.put(currentDoc.getAirsRefId(), "Vous n'êtes pas acteur de cette tâche")
          ...
          states.put(currentDoc.getAirsRefId(), "La création de l'adresse ne peut pas être validée")
        }
      }
    }
  }
...
_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW ADRESS CREATED SIMPLE VIEW INIT - END")