...
/*************************************************************************************************
 * 							    Validation étape workflow En attente d'affilié PCI - INIT
 **************************************************************************************************
 Date : 12.11.2014
 Auteur : MTO

 Description : Permet la validation du workflow en vue simple de la tâche En attente d'affilié PCI
 **************************************************************************************************/

_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW AWAITING AFFILIATE PCI SIMPLE VIEW INIT - START")
...
// Variable locale
String TASKS_VALIDATION_NAME = "TraiterAF"
...
try {
  ...
  _scriptLogger.error("[CUSTOM ACTION] - WorkflowAwaitingAffiliateEMPSimpleViewInit - ERREUR : ", e)
  return
}
...
try {
    ...
    if(Constants.DOC_LOCKED_BY_OTHER == currentDoc.getLockType()) states.put(currentDoc.getAirsRefId(), "Document vérouillé")
      ...
      for(IWFTaskModel iwfTaskModel : wfTasks) {
        ...
        if(TASKS_VALIDATION_NAME.contains(iwfTaskModel.getName())) {
          ...
          for(WFActor actor : iwfTaskModel.getActors()) {
            ...
            if(actor.getId().startsWith("airs") && tasks.get(currentDoc.getAirsRefId()) == null) {
              ...
              if(actor.getId().substring(5).equalsIgnoreCase(userContext.getUser().getId().toString())) {
                ...
                outputs.put(currentDoc.getAirsRefId(), Constants.AKAZI_OUTPUT_AFFILIATE_PCI)
                states.put(currentDoc.getAirsRefId(), "Le document peut être mis en attente affiliation PCI")
                break
              }
              else {
          ...
          states.put(currentDoc.getAirsRefId(), "Le document ne peut pas être mis en attente affiliation PCI")
        }
      }
    }
  }
  ...
  Methods.addStateMessage(data, "DATA_ERROR_MSG", "Erreur lors de l'initialisation du traitement. Veuillez contacter votre administrateur", false)
  _scriptLogger.error("[CUSTOM ACTION] - WorkflowAwaitingAffiliatePCISimpleViewInit - ERREUR : ", e)
  return
}

_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW AWAITING AFFILIATE PCI SIMPLE VIEW INIT - END")