...
/*************************************************************************************************
 * 							    Validation étape workflow Email indexé - INIT
 **************************************************************************************************
 Date : 12.11.2014
 Auteur : MTO

 Description : Permet la validation du workflow en vue simple de la tâche Email indexé
 **************************************************************************************************/

_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW EMAIL INDEXE SIMPLE VIEW INIT - START")
...
try {
  ...
  _scriptLogger.error("[CUSTOM ACTION] - WorkflowEmailIndexeSimpleViewInit - ERREUR : ", e)
  return
}
    ...
    if(Constants.DOC_LOCKED_BY_OTHER == currentDoc.getLockType()) states.put(currentDoc.getAirsRefId(), "Document vérouillé")
      ...
      for(IWFTaskModel iwfTaskModel : wfTasks) {
        ...
        if(TASKS_VALIDATION_NAME.contains(iwfTaskModel.getName())) {
          ...
          for(WFActor actor : iwfTaskModel.getActors()) {
            ...
            if(actor.getId().startsWith("airs") && tasks.get(currentDoc.getAirsRefId()) == null) {
              ...
              if(actor.getId().substring(5).equalsIgnoreCase(userContext.getUser().getId().toString())) {
                ...
                outputs.put(currentDoc.getAirsRefId(), Constants.AKAZI_OUTPUT_EMAIL_INDEXE)
                states.put(currentDoc.getAirsRefId(), "Le mail peut être défini indexé")
                break
              }
              else {
          ...
          states.put(currentDoc.getAirsRefId(), "Le mail ne peut pas être défini indexé")
        }
      }
    }
  }
...
_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW EMAIL INDEXE SIMPLE VIEW INIT - END")