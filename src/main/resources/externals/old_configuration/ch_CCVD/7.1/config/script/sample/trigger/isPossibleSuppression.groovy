...
if(airsDocument != null) {
  ...
  if(StringUtils.isNotBlank(fieldDatevalue) && StringUtils.isNotBlank(fieldDatevalue)) {
    _scriptLogger.warn("[isPossiblleSuppression] : date : " + fieldDatevalue)
    ...
    if(myToday.after(myCalendarToTcheck)) {
      ...
      _scriptLogger.warn("[isPossiblleSuppression] : suppression accept�e ")
    }
    else {
      ...
      _scriptLogger.warn("[isPossiblleSuppression] : suppression non accept�e ")
    }
    ...
    _scriptLogger.warn("fields values are null")
  }
  ...
  result.setMessageSummary(TEST_KO_SUM)
  result.setMessageDetail(TEST_KO_CAS_1_DETAIL)
  _scriptLogger.warn("airsDocument is null")
}
...
int countLinkedDoc(AirsFolder airsFolder) { // compte les documents fils li� au formulaire
  ...
  try {
    ...
    if(childIds != null) {
      ...
      _scriptLogger.warn("childIds is null")
  }
  catch(Exception e) {
    _scriptLogger.error(e.getLocalizedMessage(), e)
  }

  _scriptLogger.warn(nLinkedDOc + " child linked")
  return nLinkedDOc
