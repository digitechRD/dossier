...
_scriptLogger.debug("[CUSTOM ACTION] - DUPLICATION DOCUMENT AVIS MUTATION SIMPLE VIEW EXEC - START")
...
try {
  ...
  _scriptLogger.debug("[CUSTOM ACTION] - DuplicationDocumentAvisMutationSimpleViewExec - DEBUG - Nouveau document ID n°" + newdoc.getId().toString())
  ...
  if(!listAttach.isEmpty()) {
    ...
    if(!fattach.exists()) {
      ...
      result.setMessageSeverity(IScriptResultValueModel.Severity.ERROR)
      result.setMessageDetail("ERREUR - L'exécution du traitement est impossible. Veuillez contacter votre administrateur")
      _scriptLogger.error("[CUSTOM ACTION] - DuplicationDocumentAvisMutationSimpleViewExec - ERREUR : Téléchargement de la pièce jointe impossible")
      return
    }
    ...
    if(primaryDoc == null) {
      ...
      result.setMessageSeverity(IScriptResultValueModel.Severity.ERROR)
      result.setMessageDetail("ERREUR - L'exécution du traitement est impossible. Veuillez contacter votre administrateur")
      _scriptLogger.error("[CUSTOM ACTION] - DuplicationDocumentAvisMutationSimpleViewExec - ERREUR : Création de la nouvelle pièce jointe impossible")
      return
    }
...
_scriptLogger.debug("[CUSTOM ACTION] - DUPLICATION DOCUMENT AVIS MUTATION SIMPLE VIEW EXEC - END")
