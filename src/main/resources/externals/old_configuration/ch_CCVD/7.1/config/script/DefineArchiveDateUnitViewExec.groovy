...
_scriptLogger.debug("[CUSTOM ACTION] - DEFINE ARCHIVE DATE UNIT VIEW EXEC - START")
...
try {
  ...
  result.setMessageSummary("ACTION DATE ARCHIVAGE : ")
  ...
  _scriptLogger.error("[CUSTOM ACTION] - DefineDueDateUnitViewExec - ERREUR : ", e)
  return
}
...
try {
  ...
  if(data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
    _scriptLogger.debug("[CUSTOM ACTION] - DEFINE ARCHIVE DATE UNIT VIEW EXEC - END")
    return
  }
  ...
  if(Constants.UNLOCK_TYPE.equals(document.getLockType())) {
    ...
    _scriptLogger.debug("[CUSTOM ACTION] - DefineArchiveDateUnitViewExec - DEBUG - La date d'échéance suivante " + formatDate + " a été enregistré avec succès")
    ...
    result.setMessageDetail("ATTENTION - Le date d'échéance ne peut être enregistré car le document est vérouillé")
  }
} catch(Exception e) {
  result.setMessageSeverity(IScriptResultValueModel.Severity.ERROR)
