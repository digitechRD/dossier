...
/*************************************************************************************************
 * 							    Validation étape workflow NIP créée - INIT
 **************************************************************************************************
 Date : 12.11.2014
 Auteur : MTO

 Description : Permet la validation du workflow en vue simple de la tâche NIP créée
 **************************************************************************************************/

_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW NIP CREATED SIMPLE VIEW INIT - START")
...
// Variable locale
String TASKS_VALIDATION_NAME = "CreerNip"
...
try {
  ...
  _scriptLogger.error("[CUSTOM ACTION] - WorkflowNIPCreatedSimpleView - ERREUR : ", e)
  return
}
    ...
    if(Constants.DOC_LOCKED_BY_OTHER == currentDoc.getLockType()) states.put(currentDoc.getAirsRefId(), "Document vérouillé")
      ...
      for(IWFTaskModel iwfTaskModel : wfTasks) {
        ...
        if(TASKS_VALIDATION_NAME.contains(iwfTaskModel.getName())) {
          ...
          for(WFActor actor : iwfTaskModel.getActors()) {
            ...
            if(actor.getId().startsWith("airs") && tasks.get(currentDoc.getAirsRefId()) == null) {
              ...
              if(actor.getId().substring(5).equalsIgnoreCase(userContext.getUser().getId().toString())) {
                ...
                outputs.put(currentDoc.getAirsRefId(), Constants.AKAZI_OUTPUT_NIP_CREATED)
                states.put(currentDoc.getAirsRefId(), "Le document peut être validé pour un NIP créé")
                break
              }
              else {
          ...
          states.put(currentDoc.getAirsRefId(), "Le document ne peut pas être validé pour un NIP créé")
        }
      }
    }
  }
...
_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW NIP CREATED SIMPLE VIEW INIT - END")