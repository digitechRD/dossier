...
_scriptLogger.debug("[CUSTOM ACTION] - REALLOCATION TO SCAN SERVICE SIMPLE VIEW - START")
...
try {
  ...
  _scriptLogger.error("[CUSTOM ACTION] - RetourToScanServiceSimpleView - ERREUR : ", e)
  return
}
  ...
  for(IDocument doc : docs) {
    try {
      ...
      if(!rightMgr.isAuthorizedToEditDocument(userContext, doc)) {
        _scriptLogger.error("[CUSTOM ACTION] - RetourToScanServiceSimpleView - DOC n°" + doc.getAirsRefId() + " - Droit de modification non autorisé")
      ...
      _scriptLogger.error("[CUSTOM ACTION] - RetourToScanServiceSimpleView - DOC n°" + doc.getAirsRefId() + " - ERREUR : ", e)
...
_scriptLogger.debug("[CUSTOM ACTION] - REALLOCATION TO SCAN SERVICE SIMPLE VIEW - END")