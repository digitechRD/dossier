...
/*************************************************************************************************
 * 							    Validation étape workflow Indexation corrigée - INIT
 **************************************************************************************************
 Date : 12.11.2014
 Auteur : MTO

 Description : Permet la validation du workflow en vue simple de la tâche Indexation corrigée
 **************************************************************************************************/

_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW CORRECTED INDEX SIMPLE VIEW INIT - START")
...
// Variable locale
String TASKS_VALIDATION_NAME = "IndexAvecErreurs"
...
try {
  ...
  _scriptLogger.error("[CUSTOM ACTION] - WorkflowCorrectedIndexSimpleView - ERREUR : ", e)
  return
}
    ...
    if(Constants.DOC_LOCKED_BY_OTHER == currentDoc.getLockType()) states.put(currentDoc.getAirsRefId(), "Document vérouillé")
      ...
      for (IWFTaskModel iwfTaskModel : wfTasks) {
              ...
              for (WFActor actor : iwfTaskModel.getActors()) {
                  ...
                  if (actor.getId().startsWith("airs") && tasks.get(currentDoc.getAirsRefId()) == null) {
                      ...
                      if (actor.getId().substring(5).equalsIgnoreCase(userContext.getUser().getId().toString())) {
                          ...
                          outputs.put(currentDoc.getAirsRefId(), Constants.AKAZI_OUTPUT_CORRECTED_INDEX);
                          states.put(currentDoc.getAirsRefId(), "La correction de l'indexation peut être validée");
          break;
                      } else {
        ...
        break;
              ...
              states.put(currentDoc.getAirsRefId(), "La correction de l'indexation ne peut pas être validée");
          }
      }*/
...
_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW CORRECTED INDEX SIMPLE VIEW INIT - END")