...
_scriptLogger.debug("[CUSTOM ACTION] - DEFINE DUE DATE UNIT VIEW EXEC - START")
...
try {
  ...
  if(data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
    _scriptLogger.debug("[CUSTOM ACTION] - DEFINE DUE DATE UNIT VIEW EXEC - END")
    return
  }
  ...
  if(Constants.UNLOCK_TYPE.equals(document.getLockType())) {
    ...
    _scriptLogger.debug("La date d'échéance suivante " + formatDate + " a été enregistré avec succès")
