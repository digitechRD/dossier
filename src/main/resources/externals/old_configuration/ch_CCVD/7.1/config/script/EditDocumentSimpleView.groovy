...
//_scriptLogger.debug(">>> Starting exportToEdit.groovy...");
...
//_scriptLogger.debug("Export path is {}", ExportPath);
...
try {
  ...
  if(isLocked(xmlFile, userLogin)) {
    ...
    _scriptLogger.debug("Fichier chemin : " + fileUrl)
...
private void generateXmlFile(String file, String userLogin) {
  ...
  try {
    ...
    _scriptLogger.error("Erreur à la création du fichier xml de verrou" + e)
  }
}

private boolean isLocked(String file, String userLogin) {
  //_scriptLogger.debug("----- Test de verrou");
  ...
  if(f.exists()) {
    ...
    //_scriptLogger.debug("Le fichier de verrou existe");
    ...
    try {
      ...
      _scriptLogger.error("Erreur au lancement de la lecture du fichier xml de verrou" + e)
    }
    ...
    try {
      ...
      if(racine.getChildText("DATE").equals(today)) {
        _scriptLogger.debug("Le fichier de verrou existe en date du jour")

        if(racine.getChildText("USER").equals(userLogin)) {
          _scriptLogger.debug("Le fichier de verrou existe pour le user")
          result = false
        }
        else {
          _scriptLogger.debug("Le fichier de verrou est inexistant pour le user")
          result = true
        }
      }
      else {
        _scriptLogger.debug("Aucun verrou pour ce fichier en date du jour")
        result = false
      }
    }
    catch(Exception e) {
    ...
    _scriptLogger.debug("Le fichier de verrou est inexistant")
    result = false
  }
