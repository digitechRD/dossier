...
_scriptLogger.warn("Valeur de la categorie : " + fieldCatDocvalue)
...
while(iter.hasNext()) {
  ...
  if(StringUtils.isNotEmpty(strLabel)) {
    _scriptLogger.warn("Item : " + strLabel)

    String PremLettre = strLabel.substring(0, 1)
    _scriptLogger.warn("PremLettre : " + PremLettre)

    if(PremLettre.compareToIgnoreCase(sLettre) == 0) {
      //on fait rien
      _scriptLogger.warn("Je fais rien ")
    }
    else {
      iter.remove()
      _scriptLogger.warn("Je supprime ")
    }
  }
}
...
String getFieldValue(IField field) {
  ...
  if(values != null && !values.isEmpty()) {
    ...
    _scriptLogger.info("Field Value : " + field.getCode() + " - " + fieldvalue)
  }
  else {
    _scriptLogger.info("the field value is null or empty")
  }
