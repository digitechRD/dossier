...
_scriptLogger.debug("[CUSTOM ACTION] - EXPORT PDF UNIT VIEW EXEC - START")
...
try {
  ...
  _scriptLogger.error("[CUSTOM ACTION] - ExportPDFUnitViewExec - ERREUR : ", e)
  return
}
...
try {
  if(data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
    _scriptLogger.debug("[CUSTOM ACTION] - EXPORT PDF UNIT VIEW EXEC - END")
    return
  }
