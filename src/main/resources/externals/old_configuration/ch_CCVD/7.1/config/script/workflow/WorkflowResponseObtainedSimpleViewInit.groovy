...
/*************************************************************************************************
 * 							    Validation étape workflow Réponse obtenue - INIT
 **************************************************************************************************
 Date : 12.11.2014
 Auteur : MTO

 Description : Permet la validation du workflow en vue simple de la tâche Réponse obtenue
 **************************************************************************************************/

_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW RESPONSE OBTAINED SIMPLE VIEW INIT - START")
...
// Variable locale
String TASKS_VALIDATION_NAME = "CreerNssCICA"
...
try {
  ...
  _scriptLogger.error("[CUSTOM ACTION] - WorkflowResponseObtainedSimpleViewInit - ERREUR : ", e)
  return
}
...
try {
    ...
    if(Constants.DOC_LOCKED_BY_OTHER == currentDoc.getLockType()) states.put(currentDoc.getAirsRefId(), "Document vérouillé")
      ...
      for(IWFTaskModel iwfTaskModel : wfTasks) {
        ...
        if(TASKS_VALIDATION_NAME.contains(iwfTaskModel.getName())) {
          ...
          for(WFActor actor : iwfTaskModel.getActors()) {
            ...
            if(actor.getId().startsWith("airs") && tasks.get(currentDoc.getAirsRefId()) == null) {
              ...
              if(actor.getId().substring(5).equalsIgnoreCase(userContext.getUser().getId().toString())) {
                ...
                outputs.put(currentDoc.getAirsRefId(), Constants.AKAZI_OUTPUT_CENTRAL_RESPONSE_OK)
                states.put(currentDoc.getAirsRefId(), "La réponse au document peut être prise en compte")
                break
              }
              else {
          ...
          states.put(currentDoc.getAirsRefId(), "LLa réponse au document peut être prise en compte")
        }
      }
    }
  }
  ...
  data.put("GROOVYNAME", "WorkflowResponseObtainedSimpleViewInit")
...
_scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW RESPONSE OBTAINED SIMPLE VIEW INIT - END")