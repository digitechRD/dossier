...
import com.akazi.flowmind.api.Process
...
_scriptLogger.debug("[CUSTOM ACTION] - REINITIALIZE WORKFLOW RENTE SIMPLE VIEW - START")
...
try {
  ...
  _scriptLogger.error("[CUSTOM ACTION] - ReinitializeWorkflowRenteSimpleView - ERREUR : ", e.localizedMessage)
  return
}
...
try {
  ...
  for(IDocument doc : docs) {
    try {
      ...
      if(wfTasks.size() >= 1) {
        ...
        _scriptLogger.warn("[CUSTOM ACTION] - ReinitializeWorkflowRenteSimpleView - Attention workflow déjà existant (AIRSID : " + doc.getAirsRefId())
      }
      else {
        ...
        if(taxateur != null && taxateur.equalsIgnoreCase(userContext.getUserId())) {
          ...
          _scriptLogger.warn("[CUSTOM ACTION] - ReinitializeWorkflowRenteSimpleView - Attention taxateur non défini ou différent de l'utilisateur ayant effectué l'action (AIRSID : " + doc.getAirsRefId())
        }
      }
      ...
      _scriptLogger.error("[CUSTOM ACTION] - ReinitializeWorkflowRenteSimpleView - ERREUR lors de la réinitialisation (AIRSID : " + dos.getAirsRefId() + ") : ", e.localizedMessage)
    }
  }
  ...
  _scriptLogger.error("[CUSTOM ACTION] - ReinitializeWorkflowRenteSimpleView - ERREUR : ", e)
  result.setMessageSeverity(IScriptResultValueModel.Severity.ERROR)
  result.setMessageDetail("ERREUR - La réinitialisation est impossible. Veuillez contacter votre administrateur")
}

_scriptLogger.debug("[CUSTOM ACTION] - REINITIALIZE WORKFLOW RENTE SIMPLE VIEW - END")