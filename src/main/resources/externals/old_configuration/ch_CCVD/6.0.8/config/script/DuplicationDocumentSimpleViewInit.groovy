...
scriptLogger.debug("[CUSTOM ACTION] - DUPLICATION DOCUMENT SIMPLE VIEW INIT - START");
...
try {
    ...
    scriptLogger.error("[CUSTOM ACTION] - DuplicationDocumentSimpleViewInit - ERREUR : ",e);
    return;
}
...
try{
    ...
    Collections.sort(typeItems, new SortList());
...
scriptLogger.debug("[CUSTOM ACTION] - DUPLICATION DOCUMENT SIMPLE VIEW INIT - EMD");

public static class SortList implements Comparator<SelectItem> {
    public int compare(SelectItem value1, SelectItem value2) {      
       String label1=value1.getLabel();    
