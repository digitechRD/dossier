...
scriptLogger.debug("[CUSTOM ACTION] - SEARCH BY LIST NIP SIMPLE VIEW EXEC - START");
...
try {
    ...
    scriptLogger.error("[CUSTOM ACTION] - SearchByListNipSimpleVieweExec - ERREUR : ",e);
    return;
}
...
try{
    if (data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
        scriptLogger.debug("[CUSTOM ACTION] - SEARCH BY LIST NIP SIMPLE VIEW EXEC - END");
        return
    }
