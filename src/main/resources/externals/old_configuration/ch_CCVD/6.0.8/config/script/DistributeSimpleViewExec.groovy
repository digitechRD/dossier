...
scriptLogger.debug("[CUSTOM ACTION] - DISTRIBUTE SIMPLE VIEW EXEC - START");
...
try {
    ...
    scriptLogger.error("[CUSTOM ACTION] - DistributeSimpleViewExec - ERREUR : ", e);
    return;
}
...
try {
    if (data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
        scriptLogger.debug("[CUSTOM ACTION] - DISTRIBUTE SIMPLE VIEW EXEC - END");
        return
    }
    ...
    for (IDocument doc : docs) {
        try {
            ...
            if (Constants.UNLOCK_TYPE.equals(doc.getLockType())) {
                ...
                if (Constants.CTY_AVIS_MUTATION_DOCUMENT.equalsIgnoreCase(innerDocument.getDomain().getCode())) {
                    ...
                    if (Constants.FLAG_WORKFLOW_FINANCE.toString().equalsIgnoreCase(doc.getField(Constants.FIELD_FILENAME).getValue().toString()) || Constants.LIST_STATUS_ITEM_REQUEST_MOORED_ID.toString().equalsIgnoreCase(doc.getField(Constants.LIST_STATUS_CODE).getValue().toString())) {
                        ...
                        if (wfTasks.size() >= 1) {
                            /*scriptLogger.error("[CUSTOM ACTION] - DistributeSimpleViewExec - DOC n°" + doc.getAirsRefId() + " - Un Workflow existe déjà.");
                            */ try {
                                ...
                                scriptLogger.info("[CUSTOM ACTION] - DistributeSimpleViewExec - Suprresssion instance workflow : DOC n°" + doc.getAirsRefId());
                            } catch (Exception e) {
                                ...
                                scriptLogger.error("[CUSTOM ACTION] - DistributeSimpleViewExec - Suprresssion instance workflow : DOC n°" + doc.getAirsRefId() + " : ", e);
                                continue;
                            }
                        }
                ...
                scriptLogger.error("[CUSTOM ACTION] - DistributeSimpleViewExec - DOC n°" + doc.getAirsRefId() + " - Document verrouillé.");
            ...
            scriptLogger.error("[CUSTOM ACTION] - DistributeSimpleViewExec - DOC n°" + doc.getAirsRefId() + " : ", e);
        }
    }
