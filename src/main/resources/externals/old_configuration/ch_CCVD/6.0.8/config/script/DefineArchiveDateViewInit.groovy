...
scriptLogger.debug("[CUSTOM ACTION] - DEFINE ARCHIVE DATE VIEW INIT - START");
...
try {
    ...
    scriptLogger.error("[CUSTOM ACTION] - DefineArchiveDateViewInit - ERREUR : ",e);
    return;
}
...
try{
    ...
    scriptLogger.debug("[CUSTOM ACTION] - DefineArchiveDateViewInit - DEBUG - Date par défaut :" +calendar.getTime());
}catch(Exception e){
    Methods.addStateMessage(data, "DATA_ERROR_MSG", "Erreur lors de l'initialisation du traitement. Veuillez contacter votre administrateur", false);
    scriptLogger.error("[CUSTOM ACTION] - DefineArchiveDateViewInit - ERREUR : ",e);
    return;
...
scriptLogger.debug("[CUSTOM ACTION] - DEFINE ARCHIVE DATE VIEW INIT - END");