...
scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW REJECTED SIMPLE VIEW INIT - START");
...
try {
    ...
    scriptLogger.error("[CUSTOM ACTION] - WorkflowRejectedSimpleView - ERREUR : ",e);
    return;
}
...
try{
    ...
    for(IDocument currentDoc : docs) {
        ...
        if (Constants.DOC_LOCKED_BY_OTHER == currentDoc.getLockType()) states.put(currentDoc.getAirsRefId(), "Document vérouillé");
			...
			scriptLogger.debug(userContext.getCurrentOrgId().toString());
...
scriptLogger.debug("[CUSTOM ACTION] - WORKFLOW REJECTED SIMPLE VIEW INIT - END");