...
scriptLogger.debug("[CUSTOM ACTION] - DUPLICATION DOCUMENT SIMPLE VIEW EXEC - START");
...
try
{
    ...
    if(iSecretLevel == null){
        ...
        scriptLogger.error("[CUSTOM ACTION] - ReallocationServiceSimpleViewExec - DOC n°" + document.getAirsRefId() + " - Erreur lors de la récupération du niveau de secret : Service : "+data.get("SERVICE_LIST_VALUE").toString()+", Niveau de secret : "+iSecretLevel);
        return;
    }
    ...
    scriptLogger.debug("[CUSTOM ACTION] - DuplicationDocumentSimpleViewExec - DEBUG - Nouveau document ID n°"+newdoc.getId().toString());
    ...
    if(!listAttach.isEmpty()) {
        ...
        if (!fattach.exists()) {
            ...
            result.setMessageSeverity(IScriptResultValueModel.Severity.ERROR);
            result.setMessageDetail("ERREUR - L'exécution du traitement est impossible. Veuillez contacter votre administrateur");
            scriptLogger.error("[CUSTOM ACTION] - DuplicationDocumentSimpleViewExec - ERREUR : Téléchargement de la pièce jointe impossible");
            return;
        }
        ...
        if (primaryDoc == null) {
            ...
            result.setMessageSeverity(IScriptResultValueModel.Severity.ERROR);
            result.setMessageDetail("ERREUR - L'exécution du traitement est impossible. Veuillez contacter votre administrateur");
            scriptLogger.error("[CUSTOM ACTION] - DuplicationDocumentSimpleViewExec - ERREUR : Création de la nouvelle pièce jointe impossible");
            return;
        }
