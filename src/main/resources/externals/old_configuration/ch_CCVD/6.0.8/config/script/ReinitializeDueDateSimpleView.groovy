...
scriptLogger.debug("[CUSTOM ACTION] - REINITIALIZE DUE DATE SIMPLE VIEW - START");
...
try {
    ...
    scriptLogger.error("[CUSTOM ACTION] - ReinitializeDueDateSimpleView - ERREUR : ", e.localizedMessage);
    return;
}
...
try
{
    ...
    for(IDocument dos : docs){
        try {
            ...
            scriptLogger.error("[CUSTOM ACTION] - ReinitializeDueDateSimpleView - ERREUR lors de la réinitialisation (AIRSID : "+ dos.getAirsRefId() +") : ", e.localizedMessage);
        }
    }
...
scriptLogger.debug("[CUSTOM ACTION] - REINITIALIZE DUE DATE SIMPLE VIEW - END");