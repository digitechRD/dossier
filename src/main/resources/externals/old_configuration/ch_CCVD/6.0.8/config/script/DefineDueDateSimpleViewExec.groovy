...
scriptLogger.debug("[CUSTOM ACTION] - DEFINE DUE DATE SIMPLE VIEW EXEC - START");
...
try {
    ...
    scriptLogger.error("[CUSTOM ACTION] - DefineDueDateSimpleViewExec - ERREUR : ",e);
    return;
}
...
try{
    ...
    if (data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
        scriptLogger.debug("[CUSTOM ACTION] - DEFINE DUE DATE SIMPLE VIEW EXEC - END");
        return
    }
    ...
    for(IDocument document : docs)
    {
        if(Constants.UNLOCK_TYPE.equals(document.getLockType()))
        {
	     ...
	     scriptLogger.debug("[CUSTOM ACTION] - DefineDueDateSimpleViewExec - DEBUG - Commentaire : "+comment);
            ...
            scriptLogger.debug("[CUSTOM ACTION] - DefineDueDateSimpleViewExec - DEBUG - La date d'échéance suivante " +formatDate+ " a été enregistré avec succès pour le document : "+doc.getId());
        }
    }
