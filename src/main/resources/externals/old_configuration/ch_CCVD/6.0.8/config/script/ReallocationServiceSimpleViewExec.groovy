...
scriptLogger.debug("[CUSTOM ACTION] - REALLOCATION SERVICE SIMPLE VIEW EXEC - START");
...
try {
    ...
    scriptLogger.error("[CUSTOM ACTION] - ReallocationServiceSimpleViewExec - ERREUR : ", e.localizedMessage);
    return;
}
...
try {
    if (data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
        scriptLogger.debug("[CUSTOM ACTION] - REALLOCATION SERVICE SIMPLE VIEW EXEC - END");
        return
    }
    ...
    scriptLogger.debug("[CUSTOM ACTION] - ReallocationServiceSimpleViewExec - DEBUG - ORGANISATION WORKFLOW : "+orgaWkf);
    ...
    for (IDocument doc : docs) {
        try {
            //scriptLogger.debug("[CUSTOM ACTION] - ReallocationServiceSimpleViewExec - DEBUG - Taille de la map : "+Constants.MAP_SERVICE_SECRET_LEVEL.size()+ " - Test : "+Constants.MAP_SERVICE_SECRET_LEVEL.get(30));
			
            if(iSecretLevel == null){
                scriptLogger.error("[CUSTOM ACTION] - ReallocationServiceSimpleViewExec - DOC n°" + doc.getAirsRefId() + " - Erreur lors de la récupération du niveau de secret : Service : "+serviceId.toString()+", Niveau de secret : "+iSecretLevel);
                ...
                scriptLogger.error("[CUSTOM ACTION] - ReallocationServiceSimpleViewExec - DOC n°"+document.getAirsRefId()+" - Droit de modification non autorisé");
				...
				if((serviceId == Constants.LIST_SERVICE_ITEM_RENTES_AI_ID || serviceId == Constants.LIST_SERVICE_ITEM_RENTES_ID || serviceId == Constants.LIST_SERVICE_ITEM_RENTES_AVS_ID || serviceId == Constants.LIST_SERVICE_ITEM_API_AI_ID) && userContext.getCurrentOrgId() != Constants.ORGANIZATION_RENTES_1_ID && userContext.getCurrentOrgId() != Constants.ORGANIZATION_RENTES_2_ID && userContext.getCurrentOrgId() != Constants.ORGANIZATION_RENTES_3_ID && userContext.getCurrentOrgId() != Constants.ORGANIZATION_RENTES_4_ID){
					...
					scriptLogger.debug("[CUSTOM ACTION] - ReallocationServiceSimpleViewExec - DOC n°" + doc.getAirsRefId() + " / Oraganisation : "+orga+" / NOM : "+doc.getField(Constants.FIELD_LASTNAME_AFF_CODE).getValue());
            ...
            scriptLogger.error("[CUSTOM ACTION] - ReallocationServiceSimpleViewExec - DOC n°" + doc.getAirsRefId() + " - ERREUR : ", e);
    ...
    scriptLogger.error("[CUSTOM ACTION] - ReallocationServiceSimpleViewExec - ERREUR : ",e);
    result.setMessageSeverity(IScriptResultValueModel.Severity.ERROR);
