...
scriptLogger.debug("[CUSTOM ACTION] - DEFINE ARCHIVE DATE SIMPLE VIEW EXEC - START");
...
try {
    ...
    scriptLogger.error("[CUSTOM ACTION] - DefineArchiveDateUnitViewExec - ERREUR : ",e);
    return;
}
...
try{
    ...
    if (data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
        scriptLogger.debug("[CUSTOM ACTION] - DEFINE ARCHIVE DATE SIMPLE VIEW EXEC - END");
        return
    }
    ...
    for(IDocument document : docs){
        if(Constants.UNLOCK_TYPE.equals(document.getLockType())){
	     ...
	     //Définition commentaire
            ...
            scriptLogger.debug("[CUSTOM ACTION] - DefineArchiveDateUnitViewExec - DEBUG - La date d'archivage suivante " +formatDate+ " a été enregistré avec succès pour le document : "+doc.getId());
        }
    }
