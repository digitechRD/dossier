...
scriptLogger.debug("[CUSTOM ACTION] - DEFINE TAXING USER SIMPLE VIEW EXEC - START");
...
try {
    ...
    scriptLogger.error("[CUSTOM ACTION] - DefineTaxingUserSimpleViewExec - ERREUR : ", e.localizedMessage);
    return;
}
...
try
{
    if (data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
        scriptLogger.debug("[CUSTOM ACTION] - DEFINE TAXING USER SIMPLE VIEW EXEC - END");
        return
    }
...
{
    scriptLogger.error("[CUSTOM ACTION] - DefineTaxingUserSimpleViewExec - ERREUR : ",e);
    result.setMessageSeverity(IScriptResultValueModel.Severity.ERROR);
...
private boolean modifyTaskActorForTaxingUser(List<String> tasksList, IDocument doc, String usrId, UserContext userContext) throws Exception
    {
        ...
        if (processInstanceCollection != null){
			...
			scriptLogger.error("[CUSTOM ACTION] - DefineTaxingUserSimpleViewExec - ERREUR processInstanceCollection");
			return false;
		}

        if(processInstanceList.size() == 0) {
			scriptLogger.error("[CUSTOM ACTION] - DefineTaxingUserSimpleViewExec - ERREUR processInstanceCollection : "+processInstanceList.size());
			return false;
		}
		...
		if(tasks.isEmpty()){
			scriptLogger.error("[CUSTOM ACTION] - DefineTaxingUserSimpleViewExec - ERREUR tasksCollection : "+tasks.size());
			return false;
		}
        ...
        while( taskIte.hasNext() )
        {
			...
			if(tasksList.contains(task.getLabel()) && task.getLabel().equals("MutationCTE") && task.getProcessInstance().getCustomAttributeValue("AIRSID").equals(doc.getAirsRefId().toString()))
            {
				...
				scriptLogger.warn("[CUSTOM ACTION] - DefineTaxingUserSimpleViewExec - ERREUR processInstanceCollection : Task non trouvé : "+tasksList.toString()+" - "+task.getLabel()+" - "+task.getProcessInstance().getCustomAttributeValue("AIRSID")+" - "+tasks.size());
				//return false;
			}
        }
		return false;
