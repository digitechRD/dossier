...
scriptLogger.debug("[CUSTOM ACTION] - DEFINE ARCHIVE DATE UNIT VIEW EXEC - START");
...
try {
    ...
    if (data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
        scriptLogger.debug("[CUSTOM ACTION] - DEFINE ARCHIVE DATE UNIT VIEW EXEC - END");
        return
    }
    ...
    if (Constants.UNLOCK_TYPE.equals(document.getLockType())) {
	 ...
	 //Définition commentaire
        ...
        scriptLogger.debug("[CUSTOM ACTION] - DefineArchiveDateUnitViewExec - DEBUG - La date d'échéance suivante " + formatDate + " a été enregistré avec succès");
