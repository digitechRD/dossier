...
scriptLogger.debug("[CUSTOM ACTION] - DEFINE INFORMATIONS AVIS MUTATION SIMPLE VIEW EXEC - START");
...
try {
    ...
    scriptLogger.error("[CUSTOM ACTION] - DefineInformationsAvisMutationSimpleViewExec - ERREUR : ", e.localizedMessage);
    return;
}
...
try
{
    if (data.get("DATA_ERROR_MSG") != null || data.get("DATA_WARN_MSG") != null) {
        scriptLogger.debug("[CUSTOM ACTION] - DEFINE INFORMATIONS AVIS MUTATION SIMPLE VIEW EXEC - END");
        return
    }
...
{
    scriptLogger.error("[CUSTOM ACTION] - DefineInformationsAvisMutationSimpleViewExec - ERREUR : ",e);
    result.setMessageSeverity(IScriptResultValueModel.Severity.ERROR);
