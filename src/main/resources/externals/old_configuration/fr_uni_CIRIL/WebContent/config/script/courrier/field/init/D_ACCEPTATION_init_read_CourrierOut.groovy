...
logger.debug("Script field initialization: D_ACCEPTATION_init_read_CourrierOut.groovy --- Start");

boolean isOwnerUser = CourrierScriptUtils.isOwnerUser(usrContext, theDocument);
if (!UserUtils.hasProfile(usrContext, CourrierScriptUtils.getConstant("PROFILE_CODE_DIR")) && !isOwnerUser){
  theOutput.getValue().getProperties().put(FieldProperty.DISPLAYED, "false");
}
logger.debug("Script field initialization: D_ACCEPTATION_init_read_CourrierOut.groovy --- End");
