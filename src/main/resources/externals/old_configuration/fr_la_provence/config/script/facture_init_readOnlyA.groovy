...
logger.debug("Script field initialization: readOnly_init_factureA.groovy --- Start");

if(!UserUtils.hasProfile(usrContext, PROFIL_ADMIN_CODE)) {
    theOutput.getValue().getProperties().put(FieldProperty.READ_ONLY, "true");
}

logger.debug("Script field initialization: readOnly_init_factureA.groovy --- End");
