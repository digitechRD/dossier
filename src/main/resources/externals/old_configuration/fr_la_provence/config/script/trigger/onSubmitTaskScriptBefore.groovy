...
FAC_VALID2_CODE = LaProvenceScriptUtils.getConstant("FAC_VALID2_CODE");
...
String FACTURE_EN_TETE_VAL1_VAL2_KO = "groovy_en_tete_val1_val2_ko";
String FACTURE_KO_VAL1_VAL2 = "groovy_ko_val1_val2";
...
(FAC_NOM_SORTIE_BLOQUER.compareTo(wkfTaskSortie) == 0  || FAC_NOM_SORTIE_BAPPARTIEL.compareTo(wkfTaskSortie) == 0 )){
  ...
  User monUser = usrContext.getUser();
  ...
  if (StringUtils.isNotBlank(engNumValue))
  {
    ...
    if (documentList != null && documentList.size() == 1)
    {
      for (IDocument docEng : documentList)
      {
        ...
        //on verifie qu'il y a un valideur 1 et un valideur 2, sinon, c'est pas bon
        String sValid1 = FieldUtils.getValue(theDocument, FAC_VALID1_CODE);
        String sValid2 = FieldUtils.getValue(theDocument, FAC_VALID2_CODE);
	 String sDateEch = FieldUtils.getValue(theDocument, FAC_DATE_ECH_FIELD_CODE);

        log.debug("Script triggered onSubmitTaskScriptBefore : onSubmitTaskScriptBefore.groovy --- sValid1 : "+sValid1);
        log.debug("Script triggered onSubmitTaskScriptBefore : onSubmitTaskScriptBefore.groovy --- sValid2 : "+sValid2);
	 log.debug("Script triggered onSubmitTaskScriptBefore : onSubmitTaskScriptBefore.groovy --- sDateEch : "+sDateEch);

        
        if (StringUtils.isNotBlank(sValid1) && StringUtils.isNotBlank(sValid2) && StringUtils.isNotBlank(sDateEch))
        {
          //ScriptResultValueChecker result = new ScriptResultValueChecker();
        ...
        else
        {
          ScriptResultValueChecker result = new ScriptResultValueChecker();
          result.setMessageSeverity(  com.digitech.dossier.script.model.IScriptResultValueModel.Severity.WARN );
          result.setMessageSummary(FACTURE_EN_TETE_VAL1_VAL2_KO);
          result.setMessageDetail(FACTURE_KO_VAL1_VAL2);
          result.setValid(false);
          output.setValue(result);
          bOK = false;
          log.debug("Script triggered onSubmitTaskScriptBefore : onSubmitTaskScriptBefore.groovy --- la valideur 1 ou valideur 2 n'est pas positionné, KO");
        }
      }
