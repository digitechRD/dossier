import com.digitech.dossier.script.model.impl.result.AttachmenttoDisplayScriptResultModel;
...
AttachmenttoDisplayScriptResultModel  attachmentResultModel = new AttachmenttoDisplayScriptResultModel();  
...
IAttachment getAttachment(  List<IAttachment> attachmentsList,  currentDoc ) {
	for( IAttachment attached: attachmentsList ) {
		scriptLogger.error("current attachment  : "  + attached.getId() );
		try {
			...
			{
				scriptLogger.error("returning attachment : " + attached.getId() );
				 return attached;
			}
		}
		catch(Exception e) {
