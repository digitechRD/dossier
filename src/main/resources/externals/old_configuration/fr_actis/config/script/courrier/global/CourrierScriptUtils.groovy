...
// ####################################################################################################################################
  ...
  /**      ##ACTIS##
   * @param SocieteId the item id of Societe LA
   * @param userContext the user context
   * @return service the service code
   */
  private static String getSocieteCode(Integer SocieteId) throws IdentificationException, ServerException {
    String societe = null;
    
    if (SocieteId != null){
      Term term = ((IAuthorityList)getAuthorityListService()).getTerm(SocieteId);
      if (term == null) {
        throw new IllegalStateException("No term with ID '" + SocieteId + "' found for authority list ");
      }
      societe = term.getCode();
      return societe;
    }
  }
   ...
   * @return sequenceComplete the sequence completed with some "0"
   */
  public static String generateNumChronoActis(Integer nSociete, UserContext userContext, boolean provisory) throws IdentificationException, ServerException {
    if (nSociete == null) {
      return null;
    }
    
    ICounter counterService = ((ICounter) ServiceManager.getInstance().getService(com.digitech.dossier.common.service.Constants.SERVICE_AIRS_COUNTER_MGR));
// recupérer le code de l'item de la liste d'autorité société concerné
    String counterCode = getSocieteCode(nSociete);
    ...
    //
    //String serviceLabel = getServiceLabelFromId(service, userContext);
    String numChrono = generateNumChronoActis(counterCode, counter, userContext);
    return numChrono;
  }

  /**
   * Generate the number chrono  ##ACTIS##
   * @param service the service code
   * @param counter the counter code
   * @param userContext the user context
   * @return numChrono the number chrono
   */
  private static String generateNumChronoActis(String  societe, String counter, UserContext userContext) throws IdentificationException, ServerException {
    String numChrono = null;
    if(societe != null && counter != null) {
      String numChronoPattern = getConstant("NUM_CHRONO_PATTERN");
      String numChronoDateFormat = getConstant("NUM_CHRONO_DATE_FORMAT");
      ...
      if (numChronoPattern != null){
        numChrono = numChronoPattern.replace("<DATE>", date);
        //Actis 
        numChrono = numChrono.replace("<SOCIETE>", societe);
        numChrono = numChrono.replace("<COUNTER>", counter);
