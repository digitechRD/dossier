...
if (Boolean.TRUE.equals(Boolean.valueOf(CourrierScriptUtils.getConstant("MAIL_NOTIFICATION_ENABLED")))) {
  ...
  customPropertyMap.put("state", CourrierAdvancedAuditType.ADV_EVENT_COURRIER_CLOSED);
  ...
  ApplicationUtils.sendMail(usrContext, Arrays.asList(theDocument), templateFile, new ReportPerson(usrContext, user), BundleUtils.getTranslation("mail_subject_courrier_state_closed"), customPropertyMap);
}
