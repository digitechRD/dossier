...
// IField updatedField le champ de référence pour la déndance
...
logger.debug("Script field dependency: computeDueDate_dependency_courrierIn.groovy --- Start");

try {
  // On récupère le type du courrier
  IField courrierTypeField = usrContext.getCurrentDocument().getField(CourrierScriptUtils.getConstant("FIELD_CODE_T_TYPE"));
  if (courrierTypeField != null && courrierTypeField.getValue() != null) {
...
logger.debug("Script field dependency: computeDueDate_dependency_courrierIn.groovy --- End");
