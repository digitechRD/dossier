...
boolean required = CourrierScriptUtils.changeRequired_U_VALIDEURS(usrContext, logger, theUpdatedField, theFieldToUpdate);

List<Integer> initialValues = theFieldToUpdate.getAttributes().get("initialValues");
List<Integer> currentValues =theFieldToUpdate.getValues();
if(currentValues!=null){
  currentValues.clear();
}
if(required){
  if(currentValues==null) {
    List<Integer> values = new ArrayList<String>();
    values.addAll(initialValues);
    theFieldToUpdate.setValues(values);
  }
  else{
