...
try{
		...
		// If the document is waiting for validation nobody can change it except the current validator user
		else if(CourrierScriptUtils.getTermID(etatCourrierFieldCode, CourrierScriptUtils.getConstant("STATE_CODE_A_VALIDER")).equals(courrierState)){
			logger.debug("Le courrier est à valider");
			// To know if current user is current validator
            ...
            if (docUserValidator != null) {
				...
				// If not validator then we restrict all actions on the document
				if(!validator){
					logger.debug("L'utilisateur courant n'est pas le valideur");
					unavailableActions.clear();
            ...
            logger.debug("docUserValidator=[" + docUserValidator + "]");
		}
