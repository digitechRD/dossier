...
logger.debug("Script field initialization: readOnly_init_courrierOut.groovy --- Start");
...
//Si l'utilsiateur est un agent, les champs doivent être grisés
if(UserUtils.hasProfile(usrContext, CourrierScriptUtils.getConstant("PROFILE_CODE_AGT")))
{
	 theOutput.getValue().getProperties().put(FieldProperty.READ_ONLY, "true");
}

logger.debug("Script field initialization: readOnly_init_courrierOut.groovy --- End");
