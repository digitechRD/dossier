<jsp:root version="2.0" xmlns:f="http://java.sun.com/jsf/core"
  ...
  <a4j:outputPanel ajaxRendered="true" layout="block" styleClass="mailPanel">
    <script>
      ...
      function truc(map) {
        const details = map.closest("details");
        const checkboxes = details.querySelectorAll("input[type=checkbox][name*='attachments']");
    ...
    <h:panelGroup layout="block" rendered="#{CustomActionController.model.modalPanelModel.emailSortedAttachments.size()>0}" styleClass="row col-sm-12">
      <digi:fieldset legend="#{MessageBundleModel.modalPanelMail_label_attachment}" id="fsPJ"
        ...
        <div style="max-height:300px; overflow-y:auto; padding:2px;">
          <h:panelGroup layout="block" styleClass="row w-100 pt-2 d-flex flex-column">
            <c:forEach items="#{CustomActionController.model.modalPanelModel.emailSortedAttachments.entrySet()}" var="entry">
              <details>
                <summary style="cursor:pointer; font-weight:bold;">
                    #{entry.key}
                  <h:selectBooleanCheckbox id="test" value="#{entry.key}" onclick="truc(this)" style="margin-left: 8px;"/>
                </summary>

                <h:selectManyCheckbox id="attachments"
    ...
    <h:panelGroup layout="block" rendered="#{CustomActionController.model.modalPanelModel.emailAttachments.size()>0}" styleClass="row col-sm-12">
      <digi:fieldset legend="#{MessageBundleModel.modalPanelMail_label_attachment}" id="fsPJ"
                     styleClass="col-sm-12">
        <div style="max-height:300px; overflow-y:auto; padding:2px;">
          <h:panelGroup layout="block" styleClass="row w-100 pt-2 d-flex flex-column">
