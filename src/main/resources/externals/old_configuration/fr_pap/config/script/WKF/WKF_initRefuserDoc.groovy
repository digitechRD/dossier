...
scriptLogger.debug("In WKF_initRefuserDoc.groovy [v" + versionScript + "] [idDoc=" + document.getAirsRefId() + "]");

// Set parameter for custom panel
CustomActionModel customActionModel = Utils.getCustomActionController().getModel();
customActionModel.getModalPanelModel().put("comment","J'invalide");
...
// cette ligne de code permet d'afficher la partie saisie du commentaire 
customActionModel.getModalPanelModel().put("displayPanelComment", true);

scriptLogger.debug("Out WKF_initRefuserDoc.groovy");
