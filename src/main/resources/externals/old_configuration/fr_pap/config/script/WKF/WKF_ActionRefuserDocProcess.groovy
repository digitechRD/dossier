...
// Objet : Script appelé lors de l'action Refus de visa
// ****************************************************************************
...
// === Constantes ===
final boolean COMMENTAIRE_OBLIGATOIRE = true // ⬅️ Passe à true pour rendre le commentaire obligatoire
...
final String emailSubject = "Notification de tâche refusée - Montée sur cale : "
final String emailContentTemplate = """
Bonjour,

Nous vous informons que le dossier de la montée sur cale N°%s a été invalidé.
Il est désormais revenu à l’étape : %s.
...
final String emailSubjectCap = "Notification de tâche"
final String emailContentCap = ""
...
switch(etapeCode) {
	...
	case "controle_demande":
		nextEtape = "reception_demande" 
		FieldUtils.setValue(document, "MSC_ETAPE_EN_COURS", "${ScriptUtilsProject.getTermID("MSC_ETAPE_EN_COURS", nextEtape)}")
		FieldUtils.setValue(document, "MSC_ETAT_MONTEE", "${ScriptUtilsProject.getTermID("MSC_ETAT_MONTEE", "")}")
		break
...
//Historique
scriptLogger.debug("Workflow : refus etape "+etapeAvantLabel)
ScriptUtilsProject.getAuditMgr().addDocumentEvent(userContext, document, AdvancedAuditType.ADV_EVENT_WF_TASK_SUBMIT.name(), BundleUtils.getTranslation("WKF_ActionRefuserDocProcess")+" : Refus "+etapeAvantLabel)
