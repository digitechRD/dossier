...
try {
    ...
    if (pStart && pEnd) {
        ...
        lastDay  = PeriodBuilderHelper.computeStartingIntervalDay("D4", firstDay)
    }
    ...
    if (CollectionUtils.isNotEmpty(docIDs)) {
        ...
        String mondayRef = DateUtils.applyPattern(firstDay, Constants.FORMAT_DATE)

        for (final def row in values) {
            _logger.debug("Row dump (size={}): {}", row.size(), row)
            ...
            _logger.debug("Parsed row -> navire='{}', dateDeb='{}', dateFin='{}'", navire, ddeb, dfin)

            // Validation des dates
            boolean validDeb = false
            boolean validFin = false

            try {
                DateUtils.parseDate(ddeb, [Constants.FORMAT_DATE_TIME, Constants.FORMAT_DATE] as String[])
                validDeb = true
            } catch(Exception ignore) { }

            try {
                DateUtils.parseDate(dfin, [Constants.FORMAT_DATE_TIME, Constants.FORMAT_DATE] as String[])
                validFin = true
            } catch(Exception ignore) { }

            if (!validDeb || !validFin) {
                _logger.warn("⚠ Ligne ignorée car dates invalides pour navire='{}' -> debut='{}' fin='{}'",
                        navire, ddeb, dfin)
                continue
            }
            ...
            try {
                ...
                so.offset   = computeDateOffset(mondayRef, so.dateDeb, firstDay)

                if (so.longueur > 0) {
                    occupations.add(so)
                }
            } catch (Exception ex) {
                _logger.error("Erreur de parsing pour navire='{}' debut='{}' fin='{}' : {}",
                        navire, ddeb, dfin, ex.getMessage(), ex)
            }
        }
...
static double computeDateOffset(String startDateStr, String occStartStr, Date startDate) {
    ...
    return (double)Days.daysBetween(new DateTime(ref).withTimeAtStartOfDay(),
            new DateTime(occStart).withTimeAtStartOfDay()).getDays()
}

static List<SelectItem> buildRangeOptions() {
    [
        new SelectItem(ShipConstants.THIS_WEEK, "Cette semaine"),
        new SelectItem(ShipConstants.LAST_WEEK, "La semaine dernière"),
        new SelectItem(ShipConstants.NEXT_WEEK, "La semaine prochaine")
    ]
}
