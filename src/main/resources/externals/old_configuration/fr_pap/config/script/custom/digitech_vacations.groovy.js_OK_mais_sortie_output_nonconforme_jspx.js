...
try {
    ...
    // Vérification et valeur par défaut si vide ou null
    ...
    if (pStart != null && pEnd != null) {
        try {
            ...
            _logger.error("Erreur de parsing des dates pStart='{}' ou pEnd='{}' : {}", pStart, pEnd, e.message, e)
            throw new Exception("Format de date invalide pour dateDebut ou dateFin")
        }
    } else {
        String startPeriodKey = _model.getUserParameterValue("rangeDate", ShipConstants.THIS_MONTH, true)
        if (startPeriodKey == null || startPeriodKey.trim().isEmpty()) {
            startPeriodKey = ShipConstants.THIS_MONTH
        }
    ...
    if (CollectionUtils.isNotEmpty(docIDs)) {
        ...
        for (final def row in values) {
            if (row == null || row.size() < 3) {
                _logger.warn("Ligne de données invalide ou incomplète : {}", row)
                continue
            }
            ...
            if (ddeb.trim().isEmpty() || dfin.trim().isEmpty()) {
                _logger.warn("Dates de début ou de fin manquantes pour le navire '{}'", navire)
                continue
            }
            ...
            try {
                ...
                if (so.longueur > 0) {
                    ...
                    _logger.warn("Longueur calculée invalide pour le navire '{}' (début='{}', fin='{}')", so.navire, so.dateDeb, so.dateFin)
                }
            } catch (Exception ex) {
                _logger.error("Erreur lors du traitement du navire '{}' (début='{}', fin='{}') : {}", navire, ddeb, dfin, ex.message, ex)
            }
        }
