...
try {
	...
	def fDebut = _fieldMap.get("MSC_DATE_DEBUT")	
    //def nbJours = _fieldMap.get("MSC_DUREE_SEJOUR_CONF")?.value
	def fDuree = _fieldMap.get("MSC_DUREE_SEJOUR_CONF")
	log.debug("{}: fDebut={} | fDuree={}", SCRIPT_NAME, fDebut, fDuree)
	
	Date dtDebut = fDebut?.getValue()
	log.debug("{}: Date de début = {}", SCRIPT_NAME, dtDebut)
    def dureeRaw = fDuree?.getValue()
	log.debug("{}: Nombre de jours = {}", SCRIPT_NAME, dureeRaw)
	
	Integer nbJours = (dureeRaw instanceof Number) ? dureeRaw.intValue() : null
	
	log.debug("{}: dtDebut={} | dureeRaw={} | nbJours={}", SCRIPT_NAME, dtDebut, dureeRaw, nbJours)
