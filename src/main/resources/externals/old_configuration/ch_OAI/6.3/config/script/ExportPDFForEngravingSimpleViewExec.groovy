...
try {
    ...
    for(String s : data.get("DATA_UNDER_FILTER").toString().split("::")){
        if(s != null && !s.isEmpty()) filters.add(String.format("%05d", Integer.valueOf(s)));
    }
    ...
    Integer idFilter=0;
    if(isExportFolder) idFilter = Integer.parseInt(String.valueOf(data.get("DATA_FILTER"))) - 2;
    else idFilter =Integer.parseInt(String.valueOf(data.get("DATA_FILTER")))-1;
    ...
    if (isExportFolder) {
        if (!data.get("DATA_UNDER_FILTER").toString().isEmpty()) {
            ...
            listDocumentsExport = Methods.getDocumentsListIdByNSS(Utils.getSearchResultController().getModel().getSearchResultTableModel().getAllDocuments(), String.valueOf(data.get("DATA_FILTER_FIELD")), filters, (Date)data.get("DATA_BEGIN_DATE"), (Date) data.get("DATA_END_DATE"), false);
        }else{
            ...
            listDocumentsExport = Methods.getDocumentsListIdByNSS(Utils.getSearchResultController().getModel().getSearchResultTableModel().getAllDocuments(), String.valueOf(data.get("DATA_FILTER_FIELD")), null, (Date)data.get("DATA_BEGIN_DATE"), (Date) data.get("DATA_END_DATE"), false);
        }
    } else {
        docs = Utils.getSearchResultController().getModel().getSearchResultTableModel().getSelectedDocuments();
        for (IDocument document : docs) {
            Date dateDocument = (Date) document.getField(Constants.FIELD_DATE_DOCUMENT_CODE).getValue();
            if (data.get("DATA_UNDER_FILTER").toString().isEmpty() && (((dateDocument.after(beginDate) && dateDocument.before(endDate)) || (dateDocument.compareTo(beginDate) == 0) || (dateDocument.compareTo(endDate) == 0)))){
                listDocumentsExport.add(document.getAirsRefId());
            } else {
                for (String filter : filters) {
                    if (filter.equals(String.format("%05d", Integer.valueOf(String.valueOf(document.getField(String.valueOf(data.get("DATA_FILTER_FIELD"))).getValue())))) && !isAdmin && (((dateDocument.after(beginDate) && dateDocument.before(endDate)) || (dateDocument.compareTo(beginDate) == 0) || (dateDocument.compareTo(endDate) == 0))))
                        listDocumentsExport.add(document.getAirsRefId());
                }
            }
        }
    ...
    if (listDocumentsExport.isEmpty()) {
       ...
       serviceDistant.executeBurning(listDocumentsExport, data_tmp, xmlDocument, conf, indexDateEmissions, String.valueOf(data.get("DATA_NSS"))+"_"+userContext.getUser().getLogin()+"_"+df.format(new Date().getTime())+Constants.APPLICATION_PDF_EXTENSION);
    }

	Methods.logActionUser(Constants.ACTION_GRAVAGE,labelFilter,Integer.toString(listDocumentsExport.size()),userContext.getUser().getLogin(),data.get("DATA_NSS").toString());
