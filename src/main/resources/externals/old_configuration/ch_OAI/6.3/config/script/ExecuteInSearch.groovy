...
try {
	...
	if (currentRequest != null && !currentRequest.isEmpty()) {
		if (currentRequest.contains(Constants.FIELD_NSS_CODE+"=\"")) {
			String currentNss = currentRequest.substring(currentRequest.indexOf(Constants.FIELD_NSS_CODE+"=")+Constants.FIELD_NSS_CODE.length()+2, currentRequest.indexOf("\"", currentRequest.indexOf(Constants.FIELD_NSS_CODE+"=")+Constants.FIELD_NSS_CODE.length()+2));
		...
		if (currentRequest.contains(Constants.FIELD_GESTIONNAIRES_CODE) && !currentRequest.contains(Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE)) {
			if(finalRequest != null) currentRequest = finalRequest;
			...
			else finalRequest = currentRequest + " ET "+Constants.LIST_WK_STATUT_CODE+"=\""+Constants.LIST_WK_STATUT_TRANSFERT_PEC+"\"";
			search.setAirsRequest(finalRequest);
		}
