...
try{
    ...
    }else doc = document.getAirsDocument().getInnerDocument();
        ...
        try{
            for(Note note : doc.getNotes()) {
                for (Map.Entry<Integer, Boolean> entry : noteChecksMap.entrySet()) {
                    if(entry.getValue() && entry.getKey().equals(note.getId())) doc.deleteNote(note);
                }
            }
            Methods.getAuditMgr().addDocumentEvent(userContext, document, Constants.ADV_EVENT_COMMENT, BundleUtils.getTranslation("groovy_comment_event_deleted_unitary"));
            doc.updateContent();
        }catch(Exception e){
            if (errorDocuments == null) errorDocuments = document.getAirsRefId().toString() + " (" + BundleUtils.getTranslation("groovy_error_document") + ")";
            else errorDocuments += ", " + document.getAirsRefId().toString() + " (" + BundleUtils.getTranslation("groovy_error_document") + ")";
