    ...
    if (!listNewDocuments.isEmpty()) {
        for (Integer id : listNewDocuments) {
            ...
            try {
                ...
                Methods.defineDocumentIndex(document, Constants.FIELD_DATE_RETENTION_CODE, new SimpleDateFormat(Constants.DATE_FORMAT_AIRS).format(retentionDateDocument));
                Methods.defineDocumentIndex(document, Constants.FIELD_DATE_RETENTION_MODIFY_CODE, new SimpleDateFormat(Constants.DATE_FORMAT_AIRS).format(yesterdayDateDocument));
                ...
                if(!documentsList.isEmpty()) {
                    Document folder = new Document(DossierCoreContext.getAdminJeton(), Methods.getDocumentsListIdByNSS(Constants.CTY_FOLDER_ASSURE, Methods.getFieldValue(document, Constants.FIELD_NSS_CODE)).get(0));
                    if (Methods.getFieldValue(folder, Constants.FIELD_DATE_RETENTION_CODE) == null) {
                        ...
                        if (!datesNaissance.isEmpty() && datesNaissance != null) {
                            ...
                            Methods.defineDocumentIndex(folder, Constants.FIELD_DATE_RETENTION_FINAL_CODE, new SimpleDateFormat(Constants.DATE_FORMAT_AIRS).format(retentionDateFolder));
                            Methods.defineDocumentIndex(folder, Constants.FIELD_DATE_RETENTION_CODE, new SimpleDateFormat(Constants.DATE_FORMAT_AIRS).format(retentionDateDocument));
                            Methods.defineDocumentIndex(folder, Constants.FIELD_DATE_RETENTION_MODIFY_CODE, new SimpleDateFormat(Constants.DATE_FORMAT_AIRS).format(new Date()));
                            scriptLogger.debug("[CUSTOM ACTION] - SetRetentionDateDocumentsTask - Ajout de la date de rétention - Dossier n° " + folder.getId() + " du Document n° " + id);
                            folder.updateContent();
                        } else {
                            scriptLogger.error("[CUSTOM ACTION] - SetRetentionDateDocumentsTask - Ajout de la date de rétention - Dossier n° " + folder.getId() + " --> Date de naissance inconnu");
                        }
                    }
                }
            } catch (Exception e) {
                scriptLogger.error("[CUSTOM ACTION] - SetRetentionDateDocumentsTask - ERROR lors de l'ajout de la date de rétention pour le document n° :  " + id+" - Date de naissance : "+dateNaissance, e);
            }
        }
    }
    ...
    if(!listRemoveDocuments.isEmpty()){
        for (Integer id : listNewFolders) {
            try {
                ...
                scriptLogger.error("[CUSTOM ACTION] - SetRetentionDateDocumentsTask - ERROR lors de l'ajout de la date de rétention pour le document supprimé n° :  " + id, e);
            }
        }
    }
