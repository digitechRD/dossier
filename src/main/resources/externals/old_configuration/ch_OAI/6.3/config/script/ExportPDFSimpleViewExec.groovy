...
try {
    ...
    if (profilAuthorized.size()==0 || Methods.isActorInProfil(UserContext.getInstance().getUserId(), profilAuthorized)) {
        ...
        if (docListId.size() > 0) {
            ...
            cb.showTextAligned(PdfContentByte.ALIGN_CENTER, Methods.formatString(String.valueOf(Methods.getDocumentMgr().getDocument(UserContext.getInstance().getJeton(), docListId.get(0)).getField(Constants.FIELD_NSS_CODE).getValue()), Constants.NSS_MASK), 300, 600, 0);
            cb.endText();
            ...
            Methods.writeHeader(cb, dateTitle, nssTitle, ndemTitle, typeDocTitle, BundleUtils.getTranslation("xml_configuration_label_count_pages"), BundleUtils.getTranslation("xml_configuration_label_number_document"), y);
            ...
            for(int nbCrit=0;nbCrit<Methods.getContentsList(xmlDocument, Constants.XML_ACTIONS_REQUEST_EXPORT_FILE_COLUMN_SORTED).size();nbCrit++){
                ls.add(new SortCriterion(new Field(DossierCoreContext.getAdminJeton(), Methods.getContentsList(xmlDocument, Constants.XML_ACTIONS_REQUEST_EXPORT_FILE_COLUMN_SORTED).get(nbCrit)), ("DESC".equals(Methods.getContentsList(xmlDocument, Constants.XML_ACTIONS_REQUEST_EXPORT_FILE_OPTION_SORTED).get(nbCrit)))?Direction.DESCENDANT:Direction.ASCENDANT));
            }
                    ...
                    if (new File(converteddownPathCurrentAttachment).exists()) {
                        ...
                        if (sommaireConfig.contains(Constants.LIST_TYPES_DOCUMENT_CODE)) {
                            ...
                            if (currentDoc.getField(Constants.LIST_TYPES_DOCUMENT_CODE).getValue() != null) {
                                AuthorityListTermAdmin alTermAdmin = AuthorityListsManager.loadTerm(UserContext.getInstance().getJeton(), currentDoc.getField(Constants.LIST_TYPES_DOCUMENT_CODE).getValue());
                                if(alTermAdmin.getValue1().startsWith("al_")) typeDocument = BundleUtils.getTranslation(alTermAdmin.getValue1());
                                else typeDocument = alTermAdmin.getValue1();
                            } else {
                                typeDocument = "";
                            }
                        }
                        ...
                        if (y < 40f) {
                            ...
                            cb.addTemplate(page, 0, 0);
                            y = 600f;
                            Methods.writeHeader(cb, dateTitle, nssTitle, ndemTitle, typeDocTitle, BundleUtils.getTranslation("xml_configuration_label_page"), BundleUtils.getTranslation("xml_configuration_label_number_document"), y);
                            y = y + scaleY;
                        }
    ...
    Methods.logActionUser(Constants.ACTION_EXPORT,userContext.getUser().getLogin(),BundleUtils.getTranslation("groovy_nombre_document")+" : "+nbDocumentExporte.toString());
