			...
			if (Constants.UNLOCK_TYPE.equals(document.getLockType())) {
				List<Object> listUsers = Methods.getFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE);
					...
					if(String.valueOf(Constants.LIST_WK_STATUT_SEDEX).equalsIgnoreCase(String.valueOf(document.getField(Constants.LIST_WK_STATUT_CODE).getValue()))){
						...
						Methods.defineDocumentIndex(doc, Constants.FIELD_GESTIONNAIRES_CODE, Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap, String.valueOf(userContext.getUserId())));
						scriptLogger.debug("[CUSTOM ACTION] - DistributionSimpleViewExec - DEBUG Distribution utilisateur : " + Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap, String.valueOf(userContext.getUserId()))) + " / " + String.valueOf(userContext.getUserId());
						Methods.defineDocumentIndex(doc, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, (Integer) userContext.getUserId()));
						...
						//if (profilAdmin.getUserIds().contains(userContext.getUserId())) {
							...
							Methods.defineDocumentIndex(doc, Constants.FIELD_GESTIONNAIRES_CODE, Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap, String.valueOf(usurpedUserId)));
							scriptLogger.debug("[CUSTOM ACTION] - DistributionSimpleViewExec - DEBUG Distribution utilisateur générique (DocID : "+doc.getId()+") : " + Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap, String.valueOf(usurpedUserId))) + " / " + String.valueOf(usurpedUserId);
							Methods.defineDocumentIndex(doc, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, usurpedUserId));
						...
						if (lastSearch.indexOf(fieldToCheck) > -1) {
                            ...
                            if(listUsers.contains(Integer.parseInt(usurpedUserId))) {
                                Methods.defineDocumentIndex(doc, Constants.FIELD_GESTIONNAIRES_CODE, Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap, String.valueOf(usurpedUserId)));
                                scriptLogger.debug("[CUSTOM ACTION] - DistributionSimpleViewExec - DEBUG Distribution usurpation : " + Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap, String.valueOf(usurpedUserId))) + " / " + String.valueOf(usurpedUserId);
                                ...
                                Methods.defineDocumentIndex(doc, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, Integer.parseInt(usurpedUserId)));
                            }else{
                                if (errorDocuments == null) errorDocuments = document.getAirsRefId().toString() + " (" + BundleUtils.getTranslation("groovy_postman_no_actor") + ")";
                                else errorDocuments += ", " + document.getAirsRefId().toString() + " (" + BundleUtils.getTranslation("groovy_postman_no_actor") + ")";
                                isInError = true;
