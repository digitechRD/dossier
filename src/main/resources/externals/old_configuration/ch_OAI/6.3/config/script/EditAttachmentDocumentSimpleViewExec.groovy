            ...
            if (primaryDocument != null) {
                if (StringUtils.isExtensionIgnoreCase(primaryDocument.getFileName(), Constants.APPLICATION_PDF_EXTENSION)) {
                    ...
                    try{
                        ...
                        scriptLogger.debug(doc.getPrimaryDocumentVersions(primaryDocument).size()+" / "+primaryDocument.getFileName()+" / "+primaryDocumentDest.getFileName());
                    ...
                    /*} else if(StringUtils.isExtensionIgnoreCase(primaryDocument.getFileName(), Constants.APPLICATION_TIF_EXTENSION)) {
                    String numberPsge = null;
                    File folder = new File(ExportUtils.getExportPDFDirectory()+new Date().getTime());
                    if(!folder.exists()) folder.mkdirs();
                    File tifFile = primaryDocument.download(folder.getAbsolutePath());
                    File pdfFile = new File(tifFile.getAbsolutePath() + Constants.APPLICATION_PDF_EXTENSION.toLowerCase());
                    new TIFFOperationService().convert(tifFile, pdfFile, Collections.EMPTY_MAP);
                    File folderDest = new File(pdfFile.getParentFile().getAbsolutePath()+File.separator+"work");
                    folderDest.mkdirs();
                    String dest = folderDest.getAbsolutePath()+File.separator+file.getName();
                    pdfReader = new PdfReader(pdfFile.getAbsolutePath());
                    pdfReader.selectPages(listPagesToKeep);
                    numberPsge = String.valueOf(pdfReader.getNumberOfPages());
                    PdfStamper stamper = new PdfStamper(pdfReader, new FileOutputStream(dest));
                    stamper.close();
                    pdfReader.close();

                    PrimaryDocument primaryDocumentDest = new PrimaryDocument(pdfFile.getName(), pdfFile.getName());
                    doc.addOrUpdatePrimaryDocument(primaryDocumentDest, folderDest.getAbsolutePath());
                    doc.deletePrimaryDocument(primaryDocument);
                    Methods.getAuditMgr().addDocumentEvent(userContext, document, Constants.ADV_EVENT_ATTACHMENT_PAGE_DELETE, BundleUtils.getTranslation("groovy_edit_attachment_history_delete_page")+" : "+listPagesToDelete);
                    Methods.defineDocumentIndex(doc, Constants.FIELD_NUMBER_PAGES_CODE, numberPsge);
                    doc.updateContent();
                ...
                scriptLogger.error("[CUSTOM ACTION] - EditAttachmentDocumentSimpleViewExec - ERREUR - Document n" + document.getAirsRefId() + " ne possède pas de pièce jointe");
            }
        ...
        try {
            ...
            if (clipboardAttachmentPages != null) {
                ...
                if (fileToInsert != null) {
                    ...
                    File attachmentFile = new File(new UserCoreContext(DossierCoreContext.getAdminJeton()).getUserUploadPath() + File.separator + doc.getPrimaryDocList().get(0).getFileName());
                    ...
                    PageInterval finalPageInterval = new PageInterval(Integer.valueOf(pageInterval.getFirst().intValue() - 1), Integer.valueOf(pageInterval.getLast().intValue() - 1));
                    ...
                    Methods.getAuditMgr().addDocumentEvent(userContext, document, Constants.ADV_EVENT_WF_TASK_SUBMIT, BundleUtils.getTranslation("groovy_edit_attachment_history_insert_page") + " : " + Integer.toString(nombrePage) + " - " + BundleUtils.getTranslation("jsp_page_start") + " : " + String.valueOf(data.get("DATA_PAGES_INSERT_START_PAGE")));
                    Methods.defineDocumentIndex(doc, Constants.FIELD_NUMBER_PAGES_CODE, numberPage);
                    doc.updateContent();
        ...
        try {
            ...
            if (primaryDocument != null) {
                if (StringUtils.isExtensionIgnoreCase(primaryDocument.getFileName(), Constants.APPLICATION_PDF_EXTENSION)) {
                    ...
                    if (errorDocuments == null) {
                        Methods.getAuditMgr().addDocumentEvent(userContext, document, Constants.ADV_EVENT_WF_TASK_SUBMIT, BundleUtils.getTranslation("jsp_pages_rotation") + " : " + pageToTreat.substring(0, pageToTreat.lastIndexOf(",")) + " - " + data.get("DATA_PAGES_ROTATION_SELECTED") + "°");
                    }
