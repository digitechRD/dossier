		...
		try {
			if (document.getLockType() == Constants.DOC_LOCKED_BY_OTHER) {
				...
				if (String.valueOf(Constants.LIST_WK_STATUT_AVERIFICATION_LOT).equals(status) || String.valueOf(Constants.LIST_WK_STATUT_SEDEX).equals(status)) {
					List<Integer> validedUsers = (document.getField(Constants.FIELD_GESTIONNAIRES_CODE).getValues() == null) ? new ArrayList<Integer>() : (List<Integer>) document.getField(Constants.FIELD_GESTIONNAIRES_CODE).getValues();
					if (!validedUsers.isEmpty()) {
						...
						try {
							...
							scriptLogger.warn("[CUSTOM ACTION] - CheckSedexDocumentsSimpleViewExec - Document Sedex n°" + document.getAirsRefId()+" n'a pas été redistribué");
						}
					} else {
						String idUserConfidentiel = Methods.isConfidentiel(Methods.getFieldValue(doc, Methods.formatString(Constants.NSS_MASK, Constants.FIELD_NSS_CODE)));
						if(idUserConfidentiel != null){
							Methods.defineDocumentIndex(doc, Constants.FIELD_GESTIONNAIRES_CODE, idUserConfidentiel);
							Methods.defineDocumentIndex(doc, Constants.LIST_WK_STATUT_CODE, String.valueOf(Constants.LIST_WK_STATUT_TRANSFERT_PEC));
					...
					Methods.getAuditMgr().addDocumentEvent(userContext, document, Constants.ADV_EVENT_WF_TASK_SUBMIT, BundleUtils.getTranslation("groovy_checkSedexDocuments_history_sedex_succes"));
					...
					Methods.defineDocumentIndex(doc, Constants.LIST_WK_STATUT_CODE, String.valueOf(Constants.LIST_WK_STATUT_ARCHIVE));
					Methods.getAuditMgr().addDocumentEvent(userContext, document, Constants.ADV_EVENT_WF_TASK_SUBMIT, BundleUtils.getTranslation("groovy_checkSedexDocuments_history_transfert_succes"));
