...
try{
	...
	if (data.get("DATA_COMMENT").toString().length() == 0 || (data.get("DATA_COMMENT").toString().length() > 0 && BundleUtils.getTranslation("groovy_comment_default_message").equals(data.get("DATA_COMMENT").toString()))) {
	...
	for(IDocument document : docs)
	{
		try{
			if(!Constants.UNLOCK_TYPE.equals(document.getLockType())) {
				...
				String texte = data.get("DATA_COMMENT").toString().replace("\n", "<br />");
				String page = data.get("DATA_PAGE");
				if (page.length() > 0) texte += "<br/>" + BundleUtils.getTranslation("groovy_comment_num_page") + " :" + page;
				...
				if (!rightMgr.isAuthorizedToEditDocument(userContext, document)) {
					...
					doc.addNote(newNote);
					doc.updateContent();
					scriptLogger.debug("[CUSTOM ACTION] - AddCommentSimpleViewExec - Utilisateur avec profil consultation : "+newNote.getUserName()+" - "+newNote.getUserId());
				} else {
					newNote.setUserId(userContext.getUserId());
