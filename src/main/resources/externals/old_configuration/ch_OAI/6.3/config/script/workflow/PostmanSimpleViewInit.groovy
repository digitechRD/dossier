	...
	try {
		...
		if(document.getSecretLevel() == Constants.SECRET_LEVEL_CONFIDENTIEL){
			for(OrganizationUserAdmin user: OrganizationsManager.load(DossierCoreContext.getAdminJeton(), Constants.ORGANIZATION_OAI_ID).getUsers()){
				if(!user.getUser().getLogin().toUpperCase().contains("AIRS") && !user.getUser().getLogin().toUpperCase().contains("CONFIDENTIEL") && user.getUser().getProfilIds().contains(Constants.AIRS_PROFIL_CONFIDENTIEL)) {
			...
			for(OrganizationUserAdmin user: OrganizationsManager.load(DossierCoreContext.getAdminJeton(), Constants.ORGANIZATION_OAI_ID).getUsers()){
				if(!user.getUser().getLogin().toUpperCase().contains("AIRS") && !user.getUser().getLogin().toUpperCase().contains("CONFIDENTIEL") && user.getUser().getProfilIds().contains(Constants.AIRS_PROFIL_LFA)) {
					userCheckedMap.put(user.getUser().getId(), false);
					userLoginsMap.put(user.getUser().getId(), user.getUser().getLogin());
					userNamesMap.put(user.getUser().getId(), user.getUser().getName());
					userFirstnamesMap.put(user.getUser().getId(), user.getUser().getFirstName());
			...
			if(userContext.getCurrentOrgId() == Constants.ORGANIZATION_OAI_ID) {
				for (OrganizationUserAdmin user : OrganizationsManager.load(DossierCoreContext.getAdminJeton(), Constants.ORGANIZATION_OAI_ID).getUsers()) {
					if (!user.getUser().getLogin().toUpperCase().contains("AIRS") && !user.getUser().getLogin().toUpperCase().contains("CONFIDENTIEL") && (Constants.AIRS_PROFILS_WORKFLOW.isEmpty() || !Collections.disjoint(user.getUser().getProfilIds(), Constants.AIRS_PROFILS_WORKFLOW))) {
						userCheckedMap.put(user.getUser().getId(), false);
						userLoginsMap.put(user.getUser().getId(), user.getUser().getLogin());
						userNamesMap.put(user.getUser().getId(), user.getUser().getName());
						userFirstnamesMap.put(user.getUser().getId(), user.getUser().getFirstName());
				...
				for (OrganizationUserAdmin user : OrganizationsManager.load(DossierCoreContext.getAdminJeton(), Constants.ORGANIZATION_AVS_ID).getUsers()) {
					if(!user.getUser().getName().equalsIgnoreCase(currentProfilCode.replace("SERVICE_", "")) && "Service".equalsIgnoreCase(user.getUser().getFirstName()) && !user.getUser().getLogin().toUpperCase().contains("AIRS") && user.getActive()){
						userCheckedMap.put(user.getUser().getId(), false);
						userLoginsMap.put(user.getUser().getId(), user.getUser().getLogin());
						userNamesMap.put(user.getUser().getId(), user.getUser().getName());
						userFirstnamesMap.put(user.getUser().getId(), user.getUser().getFirstName());
