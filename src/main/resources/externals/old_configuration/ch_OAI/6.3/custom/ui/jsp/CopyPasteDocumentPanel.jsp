<jsp:root version="2.0" xmlns:f="http://java.sun.com/jsf/core"
	...
	<h:panelGroup layout="block">
		<rich:panel style="position:relative; border:none; padding-bottom: 15px; height:310px; width:574px;" rendered="#{CustomActionModel.modalPanelModel.DATA_ERROR_MSG==null}">
			<rich:panel header="#{MessageBundleModel.jsp_nss}" style="overflow:auto; width:550px; height: auto; max-height: 250px; margin-top: 2px; margin-buttom: 0px">
			...
			<rich:panel header="#{MessageBundleModel.jsp_types_document}" style="overflow:auto; width:550px; height: auto; max-height: 50px; margin-top: 2px; margin-buttom: 0px">
				<h:selectOneMenu id="typesDocument" value="#{CustomActionModel.modalPanelModel.DATA_LIST_DOCUMENT_TYPE}" rendered="#{not empty CustomActionModel.modalPanelModel.DATA_LIST_DOCUMENT_TYPES}">
                    <f:selectItems value="#{CustomActionModel.modalPanelModel.DATA_LIST_DOCUMENT_TYPES}"/>
                </h:selectOneMenu>
			</rich:panel>
			<rich:panel header="#{MessageBundleModel.jsp_ndem}" style="overflow:auto; width:550px; height: auto; max-height: 50px; margin-top: 2px; margin-buttom: 0px" rendered="#{CustomActionModel.modalPanelModel.DATA_USE_NDEM==1}">
                <h:selectOneMenu id="ndemList" editable="true" value="#{CustomActionModel.modalPanelModel.DATA_DEM}" rendered="#{not empty CustomActionModel.modalPanelModel.DATA_LIST_NDEM}">
