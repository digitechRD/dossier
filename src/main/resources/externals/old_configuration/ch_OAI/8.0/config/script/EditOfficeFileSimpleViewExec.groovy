...
try {
  ...
  if (!StringUtils.isExtensionIgnoreCase(file.getName().toUpperCase(), Constants.APPLICATION_OFFICE_WORD_EXTENSION) && !
          StringUtils.isExtensionIgnoreCase(file.getName().toUpperCase(), Constants.APPLICATION_OFFICE_EXCEL_EXTENSION)) {
    result.setValid(false)
    result.setMessageSeverity(IScriptResultValueModel.Severity.WARN)
  ...
  File treatedFile = new File(exportPathFile.getAbsolutePath() + "/AIRS_" +
          doc.getId() + "@" + Constants.APPLICATION_AIRSSERVEUR_HOST + "@" + doc.getDomain().getCode() + "@" + file.getName())
