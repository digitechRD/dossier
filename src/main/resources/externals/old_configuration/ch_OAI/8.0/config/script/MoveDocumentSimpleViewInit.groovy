...
try{
    ...
    if(docs == null || docs.size() == 0) {
        ...
        name = (docs.get(0).getField(Constants.FIELD_NAME_CODE).getValue() != null) ? docs.get(0).getField(Constants.FIELD_NAME_CODE).getValue().toString() : ""
    }else {
        for (IDocument doc : docs) {
            if (isDifferentNSS && isDifferentNDEM) break
            if(!isDifferentNSS && ("".equals(nss) || nss.equals(doc.getField(Constants.FIELD_NSS_CODE).getValue().toString()))) {
                ...
                name = (doc.getField(Constants.FIELD_NAME_CODE).getValue() != null) ? doc.getField(Constants.FIELD_NAME_CODE).getValue().toString() : ""
            }else {
                nss = ""
                name = ""
                isDifferentNSS = true
    ...
    data.put("DATA_REFRESH_WORKFLOW", false)
    String language = "fr"
