...
try {
  ...
  Map<String, List<String>> filters = Methods.getContentsMapWithFormat(xmlDocument, Constants.XML_ACTIONS_REQUEST_EXPORT_SIMPLEVIEW_FILTERS_MAP, "%05d")
...
/*            if(foldersToExclude.contains(sousDossier) && hasExcluded){
      ...
      while(it.hasNext()) {
        ...
        hasExcluded = ("0".equals(
            Methods
                .getContent(xmlDocument, Constants.XML_ACTIONS_REQUEST_EXPORT_SIMPLEVIEW_INCLUDE_FILTERS_MAP.replace("##REPLACE_VALUE##", field))
                .toString()) ? true : false)
        String values = (List<String>) filters.get(field)
        if(toAdd && ((!values.contains(String.format("%05d", Integer.valueOf(String.valueOf(doc.getField(field).getValue())))) && !hasExcluded) || (
            hasExcluded && values.contains(String.format("%05d", Integer.valueOf(String.valueOf(doc.getField(field).getValue()))))))) {
          toAdd = false
        }
    ...
    if(docListId.size() > 0) {
      ...
      PdfCopy copy = new PdfCopy(documentConcat, new FileOutputStream(exportFilesConcat))
      documentConcat.open()
      cptPage = 1
      ...
      cb.showTextAligned(PdfContentByte.ALIGN_CENTER,
                         Methods.formatString(
                             String.valueOf(
                                 Methods.getDocumentMgr().getDocument(UserContext.getInstance().getJeton(), docListId.get(0))
                                        .getField(Constants.FIELD_NSS_CODE).getValue()), Constants.NSS_MASK), 300, 600, 0)
      cb.endText()
      ...
      for(int nbCrit = 0; nbCrit < Methods.getContentsList(xmlDocument, Constants.XML_ACTIONS_REQUEST_EXPORT_FILE_COLUMN_SORTED).size(); nbCrit++) {
        ls.add(new SortCriterion(new Field(DossierCoreContext.getAdminJeton(),
                                           Methods.getContentsList(xmlDocument, Constants.XML_ACTIONS_REQUEST_EXPORT_FILE_COLUMN_SORTED).get(nbCrit)),
                                 ("DESC".equals(Methods.getContentsList(xmlDocument, Constants.XML_ACTIONS_REQUEST_EXPORT_FILE_OPTION_SORTED).get(nbCrit))) ?
          ...
          if(new File(converteddownPathCurrentAttachment).exists()) {
            ...
            if(sommaireConfig.contains(Constants.LIST_TYPES_DOCUMENT_CODE)) {
              ...
              if(currentDoc.getField(Constants.LIST_TYPES_DOCUMENT_CODE).getValue() != null) {
                AuthorityListTermAdmin alTermAdmin =
                    AuthorityListsManager.loadTerm(UserContext.getInstance().getJeton(), currentDoc.getField(Constants.LIST_TYPES_DOCUMENT_CODE).getValue())
  ...
  String lastSearch = ""
  if(userContext.getUserSpace().getPersonnalSpace().getHistorics().getSearch() != null && !
      userContext.getUserSpace().getPersonnalSpace().getHistorics().getSearch().isEmpty()) {
    lastSearch = userContext.getUserSpace().getPersonnalSpace().getHistorics().getSearch().last().getAirsRequest()
  }
  Methods.logActionUser(Constants.ACTION_EXPORT, "-", Integer.toString(docListId.size()), userContext.getUser().getLogin(),
                        Methods.getAirsRequestForUser(lastSearch, UserContext.getInstance()))
