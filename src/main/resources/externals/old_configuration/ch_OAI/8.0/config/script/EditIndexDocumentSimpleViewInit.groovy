...
try {
    ...
    for (String field : Arrays.asList(fieldsList.split("::"))) {
        if (field.startsWith("D_")) {
            ...
            data.put("DATA_FIELD_" + field, docs.get(0).getField(field).getValue())
        ...
        if (field.equalsIgnoreCase(Constants.LIST_TYPES_DOCUMENT_CODE)) {
            if (Constants.USE_GROUP_LIST) {
                ...
                data.put("DATA_AIRSDOSSIER_URL",
                        Constants.APPLICATION_AIRSDOSSIER_URL + "rest/DocSeries/al/11/link/10/" + userContext.getUser().getLogin() + "/##ID_GROUP##")
            } else
