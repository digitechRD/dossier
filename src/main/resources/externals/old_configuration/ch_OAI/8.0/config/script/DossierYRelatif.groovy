...
import com.digitech.jcorbairs.Application
import com.digitech.jcorbairs.Connection
...
String TOMCAT_SERVER = "http://ai1022i008s.gilai.oai.ch:9090/AirsDossier/faces/redirect.jsp?authentication="
String ORGID = "&orgId="
...
try {
  ...
  List<IDocument> selectedDocs =
      Utils.getSearchResultController().getModel().getSearchResultTableModel().getSelectedDocuments()

  if(selectedDocs.size() > 1) {
    throw new Exception("Plusieurs documents ont \u00e9t\u00e9 s\u00e9lectionn\u00e9s. Le dossier ne sera pas affich\u00e9.")
  }

  IDocument document = selectedDocs.get(0)

  //cr�ation du lien permettant de voir l'ensemble du dossier AIRS du document courant
  //on r�cup�re la valuer de dossier
  ...
  //on r�cup�re la chaine d'authentification de l'utilisateur courant
  ...
  //on r�cup�re l'organisation courante
  int orgid = userContext.getCurrentOrgId()
  scriptLogger.error("CURRENT ORGA : " + orgid)
  //on concatene les �l�ments pour avoir le lien
...
private Token getAdmintoken() {
  ...
  try {
    ...
    Connection co = new Connection("ai0airs5", 5000)
...
void deconnectToAirs(Token jeton) {
  scriptLogger.info("D�connexion du serveur AIRS...")
  try {
    Connection.logout(jeton)
  } catch(IdentificationException ex) {
    scriptLogger.fatal(ex.getMessage())
  }
  scriptLogger.info("D�connect� du serveur AIRS")
}
