        ...
        for(Integer id : listDocumentsToTreat) {
          try {
            ...
            if(doc.getSecretLevel() == Constants.SECRET_LEVEL_DEFAULT) {
              ...
              doc.updateContent();
              scriptLogger.
                  debug("[CUSTOM ACTION] - SetSecretLevelForConfidentialFoldersTask - Mise en confidentialité - Document n° " + id + " / Dossier n°" + nss);
            }
          } catch(Exception e) {
            scriptLogger.
                error("[CUSTOM ACTION] - SetSecretLevelForConfidentialFoldersTask - ERROR lors de l'ajout de la confidentialité pour le document n° :  " + id,
                      e);
          }
        }
          ...
          for(Integer id : listDocumentsWithoutConfidential) {
            try {
              ...
              if(doc.getSecretLevel() == Constants.SECRET_LEVEL_CONFIDENTIEL) {
                ...
                doc.updateContent();
                scriptLogger.
                    debug("[CUSTOM ACTION] - SetSecretLevelForConfidentialFoldersTask - Retrait confidentialité - Document n° " + id + " / Dossier n°" + nss);
              }
            } catch(Exception e) {
              scriptLogger.
                  error("[CUSTOM ACTION] - SetSecretLevelForConfidentialFoldersTask - ERROR lors du retrait de la confidentialité pour le document n° :  " + id,
                        e);
            }
          }
        }
