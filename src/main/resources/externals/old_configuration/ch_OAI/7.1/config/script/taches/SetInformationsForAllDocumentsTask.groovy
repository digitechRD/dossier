          ...
          if(numberPage == null) {
            ...
            try {
              ...
              if(StringUtils.isExtensionIgnoreCase(file.getName(), Constants.APPLICATION_PDF_EXTENSION)) {
                ...
                Methods.defineDocumentIndex(doc, Constants.FIELD_NUMBER_PAGES_CODE,
                                            String.valueOf(TiffImage.getNumberOfPages(new RandomAccessFileOrArray(FileUtils.readFileToByteArray(file)))))
                doc.updateContent()
              }
              file.delete()
            } catch(Exception ex) {
