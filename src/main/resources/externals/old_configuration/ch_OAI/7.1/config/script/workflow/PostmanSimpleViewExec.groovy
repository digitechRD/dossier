    ...
    try {
      ...
      if(Constants.UNLOCK_TYPE.equals(document.getLockType())) {
        ...
        if(data.get("DATA_STATUS") == null || String.valueOf(Constants.LIST_WK_STATUT_ARCHIVE).equals(data.get("DATA_STATUS").toString())) {
          scriptLogger.debug("[CUSTOM ACTION] - PostmanSimpleViewExec - DEBUG Prise en compte : Statut --> " + String.valueOf(
              document.getField(Constants.LIST_WK_STATUT_CODE).getValue()) + " / Validateur --> " + userContext.getUserId())
          if(!String.valueOf(Constants.LIST_WK_STATUT_TRANSFERT_PEC).equalsIgnoreCase(
              String.valueOf(document.getField(Constants.LIST_WK_STATUT_CODE).getValue())) &&
          ...
          else if(String.valueOf(Constants.LIST_WK_STATUT_SEDEX).equalsIgnoreCase(
              String.valueOf(document.getField(Constants.LIST_WK_STATUT_CODE).getValue())) && Constants.WORKFLOW_SEDEX_TO_ARCHIVE_AUTHORIZED) {
            Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.LIST_WK_STATUT_CODE, String.valueOf(
                Constants.LIST_WK_STATUT_ARCHIVE))
            Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_FLAG_DATE_ARCHIVE_CODE,
                                        new SimpleDateFormat(Constants.DATE_FORMAT_AIRS).format(new Date()))
            ...
            Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE,
                                        Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE,
                                                                          (Integer) userContext.getUserId()))
            ...
            if(users.isEmpty()) {
              Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.LIST_WK_STATUT_CODE, String.valueOf(
                  Constants.LIST_WK_STATUT_ARCHIVE))
              Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_FLAG_DATE_ARCHIVE_CODE, new SimpleDateFormat(Constants.
                  DATE_FORMAT_AIRS).format(new Date()))
            }
          }
            ...
            Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE,
                                        Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, usurpedUserId))
            ...
            if(users.isEmpty()) {
              Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.LIST_WK_STATUT_CODE, String.valueOf(
                  Constants.LIST_WK_STATUT_ARCHIVE))
              Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_FLAG_DATE_ARCHIVE_CODE, new SimpleDateFormat(
                  Constants.DATE_FORMAT_AIRS).format(new Date()))
            }
            /*} else {
            ...
            if(lastSearch.indexOf(fieldToCheck) > -1) {
              ...
              if(listUsers.contains(Integer.parseInt(usurpedUserId))) {
                Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, Methods.
                ...
                if(users.isEmpty()) {
                  Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.LIST_WK_STATUT_CODE,
          ...
          for(Map.Entry<Integer, Boolean> entry : userChecksMap.entrySet()) {
            if(users == null && entry.getValue())
              users = Methods.getUserName(entry.getKey())
          ...
          scriptLogger.debug("[CUSTOM ACTION] - PostmanSimpleViewExec - DEBUG Distribution : Statut --> " + String.valueOf(
              document.getField(Constants.LIST_WK_STATUT_CODE).getValue()) + " / Validateur --> " + userContext.getUserId() + " / Gestionnaires --> " + users)
          if(String.valueOf(Constants.LIST_WK_STATUT_SEDEX).equalsIgnoreCase(String.valueOf(document.getField(Constants.LIST_WK_STATUT_CODE).getValue()))) {
          ...
          else if((listUsers != null && listUsers.contains(userContext.getUserId())) || String.valueOf(Constants.LIST_WK_STATUT_GEST_INCONNU).equalsIgnoreCase(
              String.valueOf(document.getField(Constants.LIST_WK_STATUT_CODE).getValue())) || String.valueOf(Constants.LIST_WK_STATUT_ARCHIVE).equalsIgnoreCase(
              String.valueOf(document.getField(Constants.LIST_WK_STATUT_CODE).getValue())) ||
              document.getField(Constants.LIST_WK_STATUT_CODE).getValue() == null) {
            Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_GESTIONNAIRES_CODE, Methods.addValueInFieldMultiValue(
                document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap, String.valueOf(userContext.getUserId())))
            scriptLogger.debug("[CUSTOM ACTION] - PostmanSimpleViewExec - DEBUG Distribution utilisateur : " + Methods.addValueInFieldMultiValue(document,
                                                                                                                                                 Constants.
                                                                                                                                                     FIELD_GESTIONNAIRES_CODE,
                                                                                                                                                 userChecksMap,
                                                                                                                                                 String.valueOf(
                                                                                                                                                     userContext.
                                                                                                                                                         getUserId()))) + " / " +
            ...
            Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE,
                                        Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE,
                                                                          (Integer) userContext.getUserId()))
            Methods.getAuditMgr().addDocumentEvent(userContext, document, Constants.ADV_EVENT_WF_TASK_SUBMIT,
                                                   BundleUtils.getTranslation("groovy_postman_history_distribute") + " --> " + users)
          }
          else if((profilAdmin = Methods.hasActorGeneric(listUsers, userContext.getUserId())) != null) {
            //if (profilAdmin.getUserIds().contains(userContext.getUserId())) {
            Integer usurpedUserId = Methods.getUserIdBasketByProfil(profilAdmin, listUsers)
            Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_GESTIONNAIRES_CODE, Methods.addValueInFieldMultiValue(
                document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap, String.valueOf(usurpedUserId)))
            scriptLogger.debug("[CUSTOM ACTION] - PostmanSimpleViewExec - DEBUG Distribution utilisateur générique : " + Methods.addValueInFieldMultiValue(
                document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap, String.valueOf(usurpedUserId))) + " / " + String.valueOf(usurpedUserId)
            Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE,
                                        Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, usurpedUserId))
            Methods.getAuditMgr().addDocumentEvent(userContext, document, Constants.ADV_EVENT_WF_TASK_SUBMIT,
                                                   BundleUtils.getTranslation("groovy_postman_history_distribute") + " (" +
                                                       Methods.getUserName(usurpedUserId) + ") --> " + users)
            /*} else {
              if (errorDocuments == null) errorDocuments = document.getAirsRefId().toString() + " (" + BundleUtils.getTranslation("groovy_postman_no_actor") + ")";
              else errorDocuments += ", " + document.getAirsRefId().toString() + " (" + BundleUtils.getTranslation("groovy_postman_no_actor") + ")";
              isInError = true;
            ...
            if(lastSearch.indexOf(fieldToCheck) > -1) {
              ...
              usurpedUserId = usurpedUserId.substring(0, usurpedUserId.indexOf("\""))
              if(listUsers.contains(Integer.parseInt(usurpedUserId))) {
                Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_GESTIONNAIRES_CODE, Methods.
                    addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap, String.valueOf(usurpedUserId)))
                scriptLogger.debug("[CUSTOM ACTION] - PostmanSimpleViewExec - DEBUG Distribution usurpation : " + Methods.
                    addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap, String.valueOf(usurpedUserId))) + " / " +
                    String.valueOf(usurpedUserId)
                Methods.getAuditMgr().addDocumentEvent(userContext, document, Constants.ADV_EVENT_WF_TASK_SUBMIT,
                                                       BundleUtils.getTranslation("groovy_postman_history_distribute") + " (" +
                                                           Methods.getUserName(Integer.parseInt(usurpedUserId)) + ") --> " + users)
                Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, Methods.
                    addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, Integer.parseInt(usurpedUserId)))
              }
              else {
          ...
          Methods.defineDocumentIndex(document.getAirsDocument().getInnerDocument(), Constants.LIST_WK_STATUT_CODE, String.valueOf(
              Constants.LIST_WK_STATUT_TRANSFERT_PEC))
        }
        // Erreur
        else {
