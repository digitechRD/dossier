...
try {
  ...
  if(doc != null) {
    ...
    if(Constants.USE_GROUP_LIST) {
      if(document.getField(Constants.LIST_GROUPES_DOCUMENT_CODE).getValue() == null || "".
          equalsIgnoreCase(document.getField(Constants.LIST_GROUPES_DOCUMENT_CODE).getValue().toString())) {
        ...
        if(value != null)
          document.getField(Constants.LIST_GROUPES_DOCUMENT_CODE).setValue(Integer.parseInt(value))
      }
    }
  }
} catch(Exception e) {
