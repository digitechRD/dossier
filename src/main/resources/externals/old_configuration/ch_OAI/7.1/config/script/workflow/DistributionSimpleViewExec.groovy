  ...
  for(IDocument document : selectedDocs) {
    ...
    try {
      ...
      if(Constants.UNLOCK_TYPE.equals(document.getLockType())) {
        ...
        scriptLogger.debug("[CUSTOM ACTION] - DistributionSimpleViewExec - DEBUG Distribution (DocID : " + doc.getId() + ") : Statut --> " + String.valueOf(
        ...
        if(String.valueOf(Constants.LIST_WK_STATUT_SEDEX).equalsIgnoreCase(String.valueOf(document.getField(Constants.LIST_WK_STATUT_CODE).getValue()))) {
          ...
          isInError = true
            ...
            String.valueOf(Constants.LIST_WK_STATUT_GEST_INCONNU).equalsIgnoreCase(String.valueOf(
                document.getField(Constants.LIST_WK_STATUT_CODE).getValue())) ||
            String.valueOf(Constants.LIST_WK_STATUT_ARCHIVE).equalsIgnoreCase(String.valueOf(document.getField(Constants.LIST_WK_STATUT_CODE).getValue())) ||
            document.getField(Constants.LIST_WK_STATUT_CODE).getValue() == null) {
          Methods.defineDocumentIndex(doc, Constants.FIELD_GESTIONNAIRES_CODE, Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE,
                                                                                                                 userChecksMap,
                                                                                                                 String.valueOf(userContext.getUserId())))
          scriptLogger.debug("[CUSTOM ACTION] - DistributionSimpleViewExec - DEBUG Distribution utilisateur : " + Methods.addValueInFieldMultiValue(document,
                                                                                                                                                    Constants.
          ...
          Methods.getAuditMgr().addDocumentEvent(userContext, document, Constants.ADV_EVENT_WF_TASK_SUBMIT, BundleUtils.
              getTranslation("groovy_postman_history_distribute") + " --> " + users)
        }
          ...
          Methods.defineDocumentIndex(doc, Constants.FIELD_GESTIONNAIRES_CODE, Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE,
                                                                                                                 userChecksMap, String.valueOf(usurpedUserId)))
          scriptLogger.debug("[CUSTOM ACTION] - DistributionSimpleViewExec - DEBUG Distribution utilisateur générique (DocID : " +
                                 doc.getId() + ") : " + Methods.addValueInFieldMultiValue(document, Constants.FIELD_GESTIONNAIRES_CODE, userChecksMap,
          ...
          if(lastSearch.indexOf(fieldToCheck) > -1) {
            ...
            if(listUsers.contains(Integer.parseInt(usurpedUserId))) {
              Methods.defineDocumentIndex(doc, Constants.FIELD_GESTIONNAIRES_CODE, Methods.addValueInFieldMultiValue(document,
                                                                                                                     Constants.FIELD_GESTIONNAIRES_CODE,
                                                                                                                     userChecksMap,
                                                                                                                     String.valueOf(usurpedUserId)))
              scriptLogger.debug("[CUSTOM ACTION] - DistributionSimpleViewExec - DEBUG Distribution usurpation : " + Methods.
              ...
              Methods.defineDocumentIndex(doc, Constants.FIELD_GESTIONNAIRES_HISTORIQUE_CODE, Methods.addValueInFieldMultiValue(document, Constants.
                  FIELD_GESTIONNAIRES_HISTORIQUE_CODE, Integer.parseInt(usurpedUserId)))
            }
            else {
              if(errorDocuments == null)
