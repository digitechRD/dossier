...
try {
  ...
  if(!Methods.getContentsList(xmlDocument, Constants.XML_ACTIONS_REQUEST_NDEM_LIST).contains(data.get("DATA_NDEM")) && "1".
      equalsIgnoreCase(Methods.getContent(xmlDocument, Constants.XML_ACTIONS_REQUEST_GLOBAL_USE_NDEM_LIMITED_LIST))) {
    result.setMessageSeverity(IScriptResultValueModel.Severity.WARN)
  ...
  if(data.get("DATA_NDEM") != null && "1".equals(data.get("DATA_USE_NDEM").toString()) && (
      data.get("DATA_NDEM") == null || data.get("DATA_NDEM").toString().isEmpty())) {
    result.setMessageSeverity(IScriptResultValueModel.Severity.WARN)
  ...
  for(IDocument iDocument : docs) {
    if(!rightMgr.isAuthorizedToEditDocument(userContext, iDocument)) {
      ...
      if(ndem != null)
        Methods.defineDocumentIndex(docToUpdate, Constants.FIELD_DEM_CODE, ndem)
      Methods.defineDocumentIndex(docToUpdate, Constants.FIELD_NAME_CODE, name)
      Methods.getAuditMgr().addDocumentEvent(userContext, iDocument, Constants.ADV_EVENT_WF_TASK_SUBMIT, BundleUtils.getTranslation(
          "groovy_move_historisation") + " : " + Methods.formatString(iDocument.getField(Constants.FIELD_NSS_CODE).getValue().toString(), Constants.NSS_MASK))
