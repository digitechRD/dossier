...
try {
  ...
  boolean isExportFolder =
      Methods.isContentExist(xmlDocument, Constants.XML_ACTIONS_REQUEST_EXPORT_WEBAI_IS_EXPORT_FOLDER_BY_ID.replaceAll("##ID##", String.valueOf(
          data.get("DATA_FILTER"))))
  scriptLogger.debug("[CUSTOM ACTION] - EXPORT WEB@AI SIMPLE VIEW EXEC - FILTRE ID " + String.valueOf(data.get("DATA_FILTER") + " / " + Constants.
      XML_ACTIONS_REQUEST_EXPORT_WEBAI_FILTERS_BY_ID_CODE.replaceAll("##ID##", String.valueOf(data.get("DATA_FILTER")))))
  List<String> filters =
      Methods.getContentsListWithFormat(xmlDocument, Constants.XML_ACTIONS_REQUEST_EXPORT_WEBAI_FILTERS_BY_ID_CODE.replaceAll("##ID##", String.valueOf(
          data.get("DATA_FILTER"))), "%05d")
  ...
  if(!folderFile.exists())
    folderFile.mkdirs()
  scriptLogger.debug("[CUSTOM ACTION] - EXPORT WEB@AI SIMPLE VIEW EXEC - PATH XML : " +
                         data.get("DATA_INFORMATIONS_WEBAI").get("XML_WEBAI_REQUEST_EXPORT_WEBAI_XML_FILE_PATH").toString())
  ...
  if(isExportFolder && !filters.isEmpty()) {
    scriptLogger.debug(String.valueOf(data.get("DATA_NSS")) + " " +
                           filterField + " " + filters + " " + simpleDateFormat.format(beginDate) + " " + simpleDateFormat.format(endDate))
    ...
    listDocumentsExport = Methods.
        getDocumentsListByNSS(Utils.getSearchResultController().getModel().getSearchResultTableModel().getAllDocuments(), filterField, filters,
                              (Date) data.get("DATA_BEGIN_DATE"), (Date) data.get("DATA_END_DATE"), false)
  }
    ...
    for(IDocument document : docs) {
      ...
      if(filters.isEmpty()) {
        ...
        for(String filter : filters) {
          if(filter.equals(String.format("%05d", Integer.parseInt(String.valueOf(document.getField(filterField).getValue())))) && ((
              date.after(beginDate) && date.before(endDate)) || (date.compareTo(beginDate) == 0) || (date.compareTo(endDate) == 0)))
            listDocumentsExport.add(new com.digitech.jcorbairs.Document(DossierCoreContext.getAdminJeton(), document.getAirsRefId()))
        }
      }
    }
  ...
  if(!listDocumentsExport.isEmpty()) {
    ...
    ExportSedex exportSedex = new ExportSedex(data.get("DATA_INFORMATIONS_WEBAI").get("XML_WEBAI_REQUEST_EXPORT_WEBAI_OFFICE"),
                                              data.get("DATA_INFORMATIONS_WEBAI").get("XML_WEBAI_REQUEST_EXPORT_WEBAI_LOT_ID"),
                                              String.valueOf(data.get("DATA_NSS")).replaceAll("\\.", ""),
                                              data.get("DATA_INFORMATIONS_WEBAI").get("XML_WEBAI_REQUEST_EXPORT_WEBAI_UID"), folder,
                                              new SimpleDateFormat(Constants.DATE_FORMAT_SWISS).format(new Date()),
                                              new SimpleDateFormat(Constants.HOURS_FORMAT_SWISS).format(new Date()),
    ...
    for(com.digitech.jcorbairs.Document document : listDocumentsExport) {
      try {
        if(document.getPrimaryDocList().size() > 0) {
          ...
          AuthorityListTermAdmin authorityListTermAdmin = AuthorityListsManager.loadTerm(DossierCoreContext.getAdminJeton(), Integer.parseInt(
              document.getContent().getFieldValue(Constants.LIST_TYPES_DOCUMENT_CODE)))
          ...
          if(StringUtils.isExtensionIgnoreCase(pj.getName(), Constants.APPLICATION_PDF_EXTENSION)) {
            ...
            if(Math.abs((com.aspose.pdf.PageSize.getA4().getHeight() - pfi.getPageHeight(1))) > 50 || (
                Math.abs((com.aspose.pdf.PageSize.getA4().getWidth() - pfi.getPageWidth(1))) > 50)) {
              ...
              if(pj.delete())
                pdfDocument.save(export.getAbsolutePath())
              pfi.close()
            }
            else {
              if(pj.delete())
            ...
            rows.add(new Row(FilenameUtils.removeExtension(export.getName()), Constants.APPLICATION_PDF_EXTENSION.toLowerCase().replaceAll("\\.", ""),
                             Constants.APPLICATION_PDF_EXTENSION.toLowerCase().replaceAll("\\.", ""),
                             documentDateOutput, org.apache.commons.lang.StringUtils.leftPad(
                document.getContent().getFieldValue(Constants.LIST_TYPES_DOCUMENT_CODE), Constants.ID_TYPE_DOCUMENT_EXPORT_COUNT_DIGIT, "0").trim(),
                             documentTypeFrench,
            ...
            successExport++
              ...
              Methods.isContentExist(xmlDocument, Constants.XML_ACTIONS_REQUEST_EXPORT_WEBAI_WITH_EXPORT_NOT_PDF.replaceAll("##ID##", String.
                  valueOf(data.get("DATA_FILTER"))))) {
            ...
            if(warnDocuments == null)
              warnDocuments = document.getId().toString() + " (" + BundleUtils.getTranslation("groovy_format_file_invalid") + ")"
            else
              warnDocuments += ", " + document.getId().toString() + " (" + BundleUtils.getTranslation("groovy_format_file_invalid") + ")"
          }
        }
          ...
          if(errorDocuments == null)
            errorDocuments = document.getId().toString() + " (" + BundleUtils.getTranslation("groovy_empty_file") + ")"
          else
            errorDocuments += ", " + document.getId().toString() + " (" + BundleUtils.getTranslation("groovy_empty_file") + ")"
        }
      } catch(Exception e) {
        if(errorDocuments == null)
          errorDocuments = document.getId().toString() + " (" + BundleUtils.getTranslation("groovy_error_document") + ")"
        else
          errorDocuments += ", " + document.getId().toString() + " (" + BundleUtils.getTranslation("groovy_error_document") + ")"
    ...
    if(errorDocuments == null && warnDocuments == null && xml.exists()) {
      ...
      result.setMessageDetail(BundleUtils.getTranslation("groovy_traitment_success") + " - " +
                                  listDocumentsExport.size() + " " + BundleUtils.getTranslation("groovy_export_webai_export_documents"))
    }
    else if(warnDocuments != null && xml.exists()) {
      result.setMessageSeverity(IScriptResultValueModel.Severity.WARN)
      result.setMessageDetail(BundleUtils.getTranslation("groovy_traitment_success") + " - " + listDocumentsExport.size() + " " + BundleUtils.getTranslation(
          "groovy_export_webai_export_documents") + " - " + warnDocuments)
    }
    else {
      result.setMessageSeverity(IScriptResultValueModel.Severity.ERROR)
...
class ExportSedex {
  ...
  ExportSedex(String env_ged, String id_lot_sedex, String nss, String uid, String folder, String date_export, String hours_export, String supplier,
              String ecm_version, String ecm_build) {
...
class Row {
  ...
  Row(String filename, String original_extension, String current_extension, String date_document, String id_type_doc, String label_fr, String label_it,
      String label_de) {
