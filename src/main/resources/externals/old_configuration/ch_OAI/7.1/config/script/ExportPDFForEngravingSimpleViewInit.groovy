...
try {
  if(request.contains(Constants.FIELD_NSS_CODE + "=\"")) {
    ...
    if(Constants.DATE_EXPORT_END_DEFAULT != null) {
      ...
      data.put("DATA_END_DATE", new SimpleDateFormat(Constants.DATE_FORMAT_SWISS).format(new Date()))
    ...
    modeExport.
        add(new SelectItem(Constants.MODE_EXPORT_GRAVAGE_VISUALISATION_ID, BundleUtils.getTranslation(Constants.MODE_EXPORT_GRAVAGE_VISUALISATION_LIBELLE)))
    modeExport.add(new SelectItem(Constants.MODE_EXPORT_BORDEREAU_VISUALISATION_ID,
                                  BundleUtils.getTranslation(Constants.MODE_EXPORT_BORDEREAU_VISUALISATION_LIBELLE)))
    modeExport.add(new SelectItem(Constants.MODE_EXPORT_GRAVAGE_VISUALISATION_BORDEREAU_ID,
                                  BundleUtils.getTranslation(Constants.MODE_EXPORT_GRAVAGE_VISUALISATION_BORDEREAU_LIBELLE)))
    ...
    if(Utils.getSearchResultController().getModel().getSearchResultTableModel().getSelectedDocuments().size() > 0)
      ...
      filtersList = Methods.getContentsListOfSelectItem(xmlDocument, Constants.XML_ACTIONS_REQUEST_BURNS_FILTRES_TITLE_WITHOUT_SELECTED_DOCUMENTS)
