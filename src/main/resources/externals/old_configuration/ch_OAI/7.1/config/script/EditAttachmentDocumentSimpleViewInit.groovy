...
try {
  ...
  if(document.getAirsDocument().getInnerDocument().getPrimaryDocList().isEmpty())
    Methods.addStateMessage(data, "DATA_ERROR_MSG", BundleUtils.getTranslation("groovy_edit_attachment_empty"), false)
  else if(!StringUtils.isExtensionIgnoreCase(primaryDocument.getFileName(), Constants.APPLICATION_PDF_EXTENSION))
    Methods.addStateMessage(data, "DATA_ERROR_MSG", BundleUtils.getTranslation("groovy_edit_attachment_no_pdf"), false)
  else {
    ...
    if(Methods.getFieldValue(doc, Constants.FIELD_NUMBER_PAGES_CODE) == null) {
      if(!new File(ExportUtils.getExportPDFDirectory()).exists())
      ...
      numberPages = Integer.parseInt(Methods.getFieldValue(doc, Constants.FIELD_NUMBER_PAGES_CODE))
    }
    ...
    if(Utils.getClipboardController().getModel().getClipboardObjectTableModel().getRowCount() > 0) {
      ...
      for(int i = 0; i < Utils.getClipboardController().getModel().getClipboardObjectTableModel().getRowCount(); i++) {
        scriptLogger.debug("ID : " + Utils.getClipboardController().getModel().getClipboardObject(i).getId() + " - " +
                               Utils.getClipboardController().getModel().getClipboardObject(i).getIndex())
        pagesNameSelectItem.add(new SelectItem(Utils.getClipboardController().getModel().getClipboardObject(i).getId(),
                                               Utils.getClipboardController().getModel().getClipboardObject(i).getTitle()))
      }
    }
    ...
    if(!versionsListSelectItem.isEmpty())
      data.put("DATA_VERSION_ATTACHMENT", versionsListSelectItem.get(0).getValue())
