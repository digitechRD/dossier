...
logger.debug("Script field initialization: DocumentRH_init_VisibleG.groovy --- Start");

if (UserUtils.hasProfile(usrContext, "PF_ADMINISTRATION") || UserUtils.hasProfile(usrContext, "PF_MODIFICATION") || UserUtils.hasProfile(usrContext, "PF_CONSULTATION")){
	theOutput.getValue().getProperties().put(FieldProperty.DISPLAYED, "true");
}
else{
  theOutput.getValue().getProperties().put(FieldProperty.DISPLAYED, "false");
}

logger.debug("Script field initialization: DocumentRH_init_VisibleG.groovy --- End");
